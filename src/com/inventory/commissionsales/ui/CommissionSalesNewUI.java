 ...
 * Nov 5, 2014
 *//*
public class CommissionSalesNewUI extends SparkLogic {
	...
	SComboField commissionPurchaseCombo;
	...
	public SPanel getGUI() {
		...
		daoObj = new CommissionSalesNewDao();
		saleDao = new SalesNewDao();
		...
		try {
			...
			commissionPurchaseCombo = new SComboField(null, 125,new CommissionPurchaseDao().getAllActivePurchaseNos(getOfficeID()), "id", "comments");
			commissionPurchaseCombo.setInputPrompt(getPropertyName("select"));
			...
			list.add(new CommissionSalesNewModel(0, "----Create New-----"));
			...
			masterDetailsGrid.setColumnExpandRatio(1, 2.3f);
			...
			masterDetailsGrid.addComponent(new SLabel(
					getPropertyName("Commission Purchase No")), 0, 2);
			masterDetailsGrid.addComponent(commissionPurchaseCombo, 2, 2);
			...
			unitMapButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					if (itemSelectCombo.getValue() != null) {
						ItemModel itm = null;
						try {
							itm = saleDao.getItem((Long) itemSelectCombo
									.getValue());
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
...
//			});
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (isValid()) {
							...
							CommissionSalesNewModel salObj = new CommissionSalesNewModel();

							List<CommissionSalesDetailsNewModel> itemsList = new ArrayList<CommissionSalesDetailsNewModel>();
							...
							while (it.hasNext()) {
								invObj = new CommissionSalesDetailsNewModel();
							...
							salObj.setCommission_sales_list(itemsList);
							salObj.setSales_local_type(((Integer) salesLocalTypeField
									.getValue()));
							salObj.setPurchase_id(toLong(commissionPurchaseCombo.getValue().toString()));
								...
								if (settings.isUSE_SALES_NO_IN_SALES_ORDER()) {
									if (ordrNo_to_salNo)
										...
										salObj.setSales_number(getNextSequence(
											"Commission Sales Number", getLoginID()));
								} else
									salObj.setSales_number(getNextSequence(
										"Commission Sales Number", getLoginID()));
							...
							long id = daoObj.save(salObj, trans.getTransaction(
									SConstants.COMMISSION_SALES, CommonUtil
											.getSQLDateFromUtilDate(date
													.getValue())));

							saveActivity(
									getOptionId(),
									"New Commission Sales Created. Bill No : "
						...
						public void valueChange(ValueChangeEvent event) {
							try {
								...
								if (salesNumberList.getValue() != null
									...
									CommissionSalesNewModel salObj = daoObj
									...
									double netTotal;
									Iterator it = salObj
											.getCommission_sales_list()
											.iterator();
									GradeModel grd;
									String grdName = "";
									while (it.hasNext()) {
										invObj = (CommissionSalesDetailsNewModel) it
												.next();
									...
									responsibleEmployeeCombo.setValue(salObj
											.getResponsible_person());
									commissionPurchaseCombo.setValue(salObj
											.getPurchase_id());
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (isValid()) {
							...
							saveActivity(
									getOptionId(),
									"Commission Sales Updated. Bill No : "
											+ salObj.getSales_number()
											+ ", Customer : "
											+ customerSelect
													.getItemCaption(customerSelect
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if (isUpdatingValid()) {
							...
							ItemModel itm = saleDao.getItem((Long) itemSelectCombo.getValue());
			...
			printButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						double netTotal = 0;
						CommissionSalesDetailsNewModel invObj;
						Iterator<CommissionSalesDetailsNewModel> itr1 = salObj.getCommission_sales_list().iterator();
						while (itr1.hasNext()) {
							invObj = itr1.next();
			...
			stockSelectList.addValueChangeListener(new ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						if (stockSelectList.getValue() != null) {
							CommissionStockModel stk = daoObj
			...
			public void valueChange(ValueChangeEvent event) {
				try {
					...
					if (barcode.trim().length() > 0) {
						itemSelectCombo.setComponentError(null);
						stockSelectList.setComponentError(null);
						CommissionStockModel mdl = daoObj.getItemFromBarcode(barcode);
						if (mdl != null) {
	...
	protected void resetUnit() {
		try {
			...
			if (itemSelectCombo.getValue() != null) {
				ItemModel itm = saleDao.getItem((Long) itemSelectCombo
						.getValue());
	...
	public Boolean isValid() {
		...
		if (commissionPurchaseCombo.getValue() == null
				|| commissionPurchaseCombo.getValue().equals("")) {
			setRequiredError(commissionPurchaseCombo,
					getPropertyName("invalid_selection"), true);
	...
	public void removeAllErrors() {
		...
		if (responsibleEmployeeCombo.getComponentError() != null)
			setRequiredError(responsibleEmployeeCombo, null, false);
		if (commissionPurchaseCombo.getComponentError() != null)
			setRequiredError(commissionPurchaseCombo, null, false);
	}
