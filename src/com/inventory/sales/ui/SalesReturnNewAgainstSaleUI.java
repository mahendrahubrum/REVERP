 ...
 * @Date Jan 9, 2014
 */
public class SalesReturnNewAgainstSaleUI extends SparkLogic {
	...
	static String TBC_STOCK_QTY = "Stock Qty";
	...
	SComboField itemsCompo, salesBillComboField;
	...
	STextField stkQtyTextField, wasteQtyTextField, returnQtyTextField,
			purchRateTextField;
	...
	public SPanel getGUI() {
		...
		allHeaders = new String[] { TBC_SN, TBC_ITEM_ID, TBC_ITEM_CODE,
				...
				TBC_QTY_IN_BASIC_UNI, TBC_STOCK_QTY, TBC_WASTE_QTY,
				TBC_PURCHASE_RETURN_QTY, TBC_SUPPLIER_ID, TBC_PURCHASE_RATE };

		if (taxEnable) {
			requiredHeaders = new String[] { TBC_SN, TBC_ITEM_CODE,
					TBC_ITEM_NAME, TBC_QTY, TBC_STOCK_QTY, TBC_WASTE_QTY,
					TBC_PURCHASE_RETURN_QTY, TBC_UNIT, TBC_UNIT_PRICE,
		...
		try {
			...
			List lists = daoObj.getSalesBillNo((long) 0, getOfficeID());
			salesBillComboField = new SComboField(null, 120, lists, "id",
					"comments");

			stkQtyTextField = new STextField(getPropertyName("stock_qty"), 60);
			...
			masterDetailsGrid.addComponent(salesBillComboField, 4, 0);
			...
			masterDetailsGrid.setComponentAlignment(salesBillComboField,
			...
			unitPriceTextField = new STextField(
					getPropertyName("sales_return_unit_price"), 100);
			...
			table.addContainerProperty(TBC_PURCHASE_RETURN_QTY, Double.class,
					null, getPropertyName("purchase_qty"), null, Align.RIGHT);
			table.addContainerProperty(TBC_SUPPLIER_ID, Long.class, null,
					TBC_SUPPLIER_ID, null, Align.CENTER);
...
//							try {
//								if (salesBillComboField.getValue() != null
//										&& !salesBillComboField.getValue()
//												.toString().equals("0")) {
//									SalesModel salObj = new SalesDao()
//											.getSale((Long) salesBillComboField
//													.getValue());
//
						...
						public void valueChange(ValueChangeEvent event) {
							...
							try {
								if (customerSelect.getValue() != null) {
									...
									loadDataInBillNoCombo((Long) customerSelect
											.getValue());
...
//									.getValue()));
							...
							purObj.setSales_numbers(salesBillComboField
									.getValue() + "");
...
//									// Date());
//									salesBillNoTextField.setValue(null);
					...
					try {
						...
						if (isAddingValid()) {
							...
							table.addItem(new Object[] {
									table.getItemIds().size() + 1, itm.getId(),
									itm.getItem_code(), itm.getName(), qty,
									objUnit.getId(), objUnit.getSymbol(),
									toDouble(unitPriceTextField.getValue()),
									objTax.getId(), tax_amt, tax_perc,
									discount_amt, totalAmt, (long) 0, (long) 0,
									conv_rat, conv_rat * qty, stkQty, wasteQty,
...
//						map.put("CUSTOMER_NAME", customerModel.getName());
		...
		try {
			...
			if (ret) {
				...
				if ((toDouble(returnQtyTextField.getValue())
						+ toDouble(wasteQtyTextField.getValue()) + toDouble(stkQtyTextField
	...
	protected void loadDataInBillNoCombo(long customerId) {
		List testList;
		try {

			testList = daoObj.getSalesBillNo(customerId, getOfficeID());

			if (testList == null)
				testList = new ArrayList();

			salesBillComboField.setInputPrompt("-----Select-----");

			SCollectionContainer container = SCollectionContainer.setList(
					testList, "id");
			salesBillComboField.setContainerDataSource(container);
			salesBillComboField.setItemCaptionPropertyId("comments");

			salesBillComboField.setValue(0);
