...
import com.inventory.sales.model.GrvSalesExpenseDetailsModel;
...
public class GRVSalesUI extends SparkLogic {
	...
	public SPanel getGUI() {
		...
		allHeaders = new String[] { TBC_SN, TBC_ID, TBC_ITEM_ID, TBC_ITEM_CODE, TBC_ITEM_NAME
				, TBC_QTY, TBC_CONVERTION_QTY, TBC_QTY_IN_BASIC_UNIT,  
				TBC_UNIT_ID, TBC_UNIT, TBC_UNIT_PRICE, TBC_CID, TBC_CURRENCY, TBC_CONV_RATE,
				TBC_DISCOUNT, TBC_TAX_ID, TBC_TAX_PERCENTAGE, TBC_TAX_AMOUNT, TBC_CESS,
				TBC_NET_PRICE,  TBC_GRADE_ID, TBC_GRADE, TBC_LOCATION_ID, TBC_LOCATION, 
				TBC_BATCH_ID, TBC_STOCK_ID};
		
		
			requiredHeaders = new String[] { TBC_SN, TBC_ITEM_CODE, TBC_ITEM_NAME, TBC_QTY, 
					/* TBC_QTY_ORDERED,TBC_BALANCE, */ TBC_UNIT, TBC_UNIT_PRICE, TBC_CURRENCY ,TBC_DISCOUNT, TBC_TAX_AMOUNT, TBC_CESS,  
		...
		try {
			...
			stockSelectList.setWidth("500px");
			...
			locationSubList.add(0, new LocationModel(0, "ALL"));
...
//			itemLayout.addComponent(locationCombo, 12, 1);
			...
			changeStkButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					if (itemCombo.getValue() != null && unitSelect.getValue() != null) {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						if(locationCombo.getValue()!=null && itemCombo.getValue()!=null){
							...
							lst = comDao.getGRVStocksInLocation(
								(Long) itemCombo.getValue(),
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (salesOrderOptions.getValue() != null && ((Set<Long>) salesOrderOptions.getValue()).size() > 0) {
							...
							List orderList=new ArrayList();
							orderList=delDao.getAllDataFromSalesOrder(salesOrders);
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						if (itemCombo.getValue() != null) {
							...
							stockSelectList.setItemCaptionPropertyId("details");
							long stk_id = comDao.getDefaultStockToSelect((Long) itemCombo.getValue());
							if (stk_id != 0)
								stockSelectList.setValue(stk_id);
							else {
							...
							if(settings.isDISCOUNT_ENABLE()){
								discountField.setValue(itmDao.getItemDiscount((Long) itemCombo.getValue())+"");
							}
						}
						else{
							convertionQuantityField.setValue("1");
						...
						if (unitSelect.getValue() != null) {
							...
							if (itemCombo.getValue() != null) {
								...
								if (quantityField.getValue() != null && !quantityField.getValue().toString().equals("0")) {
									...
									netPriceField.setNewValue(asString(roundNumber(unitPriceField.getValue()*toDouble(quantityField.getValue())-disc)));
								}
							}
						}
					} catch (Exception e) {
					...
					try {
						if (isAddingValid()) {
							...
							while (itr.hasNext()) {
								...
								idList.add(toLong(item.getItemProperty(TBC_STOCK_ID).getValue().toString()));
							}
							...
							if(!idList.contains((Long)stockSelectList.getValue())){
								table.addItem(new Object[]{
										table.getItemIds().size()+1,
										(long)0,
										itemModel.getId(),
										...
										(Long)stockSelectList.getValue()},table.getItemIds().size()+1);
							}
							else{
								Notification.show(getPropertyName("change_stock"), Type.WARNING_MESSAGE);
							}
							resetItems();
							calculateTotals();
							table.setVisibleColumns(requiredHeaders);
					...
					try {
						if (isAddingValid()) {
							...
							while (itr.hasNext()) {
								...
								idList.add(toLong(itm.getItemProperty(TBC_STOCK_ID).getValue().toString()));
							}
							Collection selectedItems = null;
							if (table.getValue() != null) {
								selectedItems = (Collection) table.getValue();
							...
							if (selectedItems != null && selectedItems.size() == 1) {
								Item item = table.getItem(selectedItems.iterator().next());
								long id=(Long)item.getItemProperty(TBC_STOCK_ID).getValue();
								if(id!=0)
									idList.remove(id);
								ItemModel itemModel = itemDao.getItem((Long) itemCombo.getValue());
								UnitModel unitModel = untDao.getUnit((Long) unitSelect.getValue());
				...
				public void buttonClick(ClickEvent event) {
					try {
						if(isValid()){
							...
							GrvSalesModel mdl=new GrvSalesModel();
							List<GrvSalesInventoryDetailsModel> itemsList = new ArrayList<GrvSalesInventoryDetailsModel>();
							Iterator it = table.getItemIds().iterator();
							while (it.hasNext()) {
								GrvSalesInventoryDetailsModel det=new GrvSalesInventoryDetailsModel();
								Item item = table.getItem(it.next());
...
//								}
								...
								while (itr.hasNext()) {
									...
									GrvSalesExpenseDetailsModel det=new GrvSalesExpenseDetailsModel();
			...
			salesNoCombo.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						if(salesNoCombo.getValue()!=null && !salesNoCombo.getValue().toString().equals("0")){
							table.setVisibleColumns(allHeaders);
							GrvSalesModel mdl=dao.getSalesModel((Long)salesNoCombo.getValue());
...
//									
							...
							while (itr.hasNext()) {
								GrvSalesExpenseDetailsModel det = (GrvSalesExpenseDetailsModel) itr.next();
						...
						if (salesNoCombo.getValue() != null && !salesNoCombo.getValue().toString().equals("0")) {
								ConfirmDialog.show(getUI(), "Are you sure?", new ConfirmDialog.Listener() {
									public void onClose(ConfirmDialog dialog) {
										if (dialog.isConfirmed()) {
											try {
//												GrvSalesModel mdl=dao.getSale((Long)salesNoCombo.getValue());
...
//							report.setExportReport(false);
	...
	public void calculateTotals() {
		try {
			double qty_ttl = 0, net_ttl = 0, discount=0;
			Item item;
			Iterator it = table.getItemIds().iterator();
			while (it.hasNext()) {
				item = table.getItem(it.next());
	...
	public boolean isAddingValid() {
		...
		try {
			...
			double qty=0;
			try {
				qty=toDouble(quantityField.getValue().toString().trim());
			} catch (Exception e) {
				qty=0;
			}
			...
			double conv=0;
			try {
				conv=toDouble(convertionQuantityField.getValue().toString().trim());
			} catch (Exception e) {
				conv=0;
			}
			...
			try {
				if(stockSelectList!=null)
					stock=itemDao.getItemStockModel((Long)stockSelectList.getValue());
			} catch (Exception e) {
			}
			...
			try {
				if (selectedItems != null && selectedItems.size() == 1) {
					Item item = table.getItem(selectedItems.iterator().next());
					qtyC=(Double)item.getItemProperty(TBC_QTY_IN_BASIC_UNIT).getValue();
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
			
			System.out.println("Tble "+qtyC);
			
			if(stock!=null){
				if((qty*conv)>(stock.getBalance()+qtyC)){
					setRequiredError(quantityField, getPropertyName("invalid_data"), true);
					ret = false;
				}
			}
	...
	public void loadOptions(long id) {
		List list = new ArrayList();
		try {
			list.add(new GrvSalesModel(0, "----Create New-----"));
	...
	public void calculateNetPrice(){
		...
		if (discountField.getValue() == null || discountField.getValue().toString().equals("")) {
			discountField.setValue("0");
		} else {
			try {
		...
		double netPrice;
		try {
			netPrice = toDouble(quantityField.getValue().toString())*unitPriceField.getValue()-Double.parseDouble(discountField.getValue());
		} catch (Exception e) {
			netPrice=0;
		}
