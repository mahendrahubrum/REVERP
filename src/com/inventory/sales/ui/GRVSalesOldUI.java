...
import com.inventory.sales.dao.GRVSalesOldDao;
...
public class GRVSalesOldUI extends SparkLogic {
	...
	public SPanel getGUI() {
		...
		allHeaders = new String[] { TBC_SN, TBC_ITEM_ID, TBC_ITEM_CODE,
				...
				TBC_STOCK_ID, TBC_CONVERTION_QTY };
		...
		try {
			...
			list.addAll(daoObj.getAllGRVSalesNumbersAsComment(getOfficeID()));
				...
				public void valueChange(ValueChangeEvent event) {
					...
					try {
						...
						if (customerSelect.getValue() != null)
							creditPeriodTextField.setValue(asString(custDao
									.getCustomerMaxCRLimitFromLedger((Long) customerSelect
											.getValue())));

						/*
						 * if (session.getAttribute("SO_Select_Disabled") ==
						 * null) {
						 * 
						 * 
						 * Iterator it=table.getItemIds().iterator(); List
						 * delList=new ArrayList(); while (it.hasNext()) {
						 * Object obj=it.next(); Item item = table.getItem(obj);
						 * if((Long)
						 * item.getItemProperty(TBC_PO_ID).getValue()>0){
						 * delList.add(obj); } } Iterator
						 * it1=delList.iterator(); while(it1.hasNext()) {
						 ...
						 * if (customerSelect.getValue() != null) {
						 * 
						 * List list = daoObj .getAllSalesOrdersForCustomer(
						 * (Long) customerSelect.getValue(), getOfficeID());
						 * 
						 * if (list.size() > 0) {
						 * 
						 * SCollectionContainer bic = SCollectionContainer
						 * .setList(list, "id"); salesOrdersOptions
						 * .setContainerDataSource(bic); salesOrdersOptions
						 ...
						 * setRequiredError(salesOrdersOptions, null, false); }
						 * else { setRequiredError( salesOrdersOptions,
						 * "No SO is available for this Ledger", true);
						 * addSOButton.focus(); }
						 * 
						 * } } else { setRequiredError(table, null, false); }
						 * 
						 * } else {
					...
					/*try {
						...
						if (isValid()) {
							...
							salObj.setSales_person((Long) employSelect
									.getValue());
							salObj.setSales_local_type(SConstants.local_foreign_type.LOCAL);
			...
			salesNumberList.addListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					/*try {
						...
						System.out.println(salesNumberList.getValue());
						if (salesNumberList.getValue() != null
							...
							employSelect.setValue(salObj.getSales_person());
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						if (itemSelectCombo.getValue() != null) {
							...
							List lst = daoObj.getStocks((Long) itemSelectCombo
									.getValue());
							...
							// long stk_id=comDao.getDefaultStockToSelect((Long)
							// itemSelectCombo.getValue());
	...
	public void calculateTotals() {
		try {
			...
			table.setColumnFooter(TBC_QTY, asString(new BigDecimal(
					roundNumber(qty_ttl))));
	...
	public boolean isUpdatingValid() {
		...
		try {
			...
			// stockSelectList.getValue().equals("")) {
			// setRequiredError(stockSelectList,
			// "Select a Stock. There may no GRV stock.", true);
