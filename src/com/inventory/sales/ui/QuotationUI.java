...
import com.inventory.sales.bean.SalesQuotationBean;
 ...
 * @Project REVERP
 */
public class QuotationUI extends SparkLogic {
	...
	QuotationDao dao;
	SComboField salesQuotationCombo;
	...
	SOptionGroup salesInquiryOptions;	
	SWindow salesInquiryWindow;
	...
	public SPanel getGUI() {
		...
		try {
			...
			salesQuotationCombo = new SComboField(null, 150, null, "id","ref_no", false, getPropertyName("create_new"));
			...
			salLisrLay.addComponent(salesQuotationCombo);
			...
			importButton = new SButton(null, "Import From Sales Inquiry");
			...
			salesInquiryOptions = new SOptionGroup("Select Sales Inquiry : ", 300, null, "id", "name", true);
			salesInquiryWindow = new SWindow("Sales Inquiry", 650, 250);
			salesInquiryWindow.center();
			salesInquiryWindow.setResizable(false);
			salesInquiryWindow.setModal(true);
			
			SFormLayout popUpLayout = new SFormLayout();
			popUpLayout.setSpacing(true);
			popUpLayout.setMargin(true);
			popUpLayout.addComponent(salesInquiryOptions);
			popUpLayout.addComponent(addInquiryButton);
			salesInquiryWindow.setContent(popUpLayout);
			...
			ClickListener confirmListener=new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(event.getButton().getId().equals("1")) {
						try {
							saveReportedIssue(getOptionId(),confirmBox.getComments(),(Long)salesQuotationCombo.getValue(),confirmBox.getUserID());
			...
			ClickListener clickListnr=new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(event.getButton().getId().equals(windowNotif.SAVE_SESSION)) {
						if(salesQuotationCombo.getValue()!=null && !salesQuotationCombo.getValue().toString().equals("0")) {
							saveSessionActivity(getOptionId(), (Long)salesQuotationCombo.getValue(),
									"sales Order : No. "+salesQuotationCombo.getItemCaption(salesQuotationCombo.getValue()));
							Notification.show("Success",
									"Session Saved Successfully..!",
									Type.WARNING_MESSAGE);
						}
			...
			createNewButton.addClickListener(new ClickListener() {
				@Override
				public void buttonClick(ClickEvent event) {
					salesQuotationCombo.setValue((long) 0);
					salesQuotationCombo.setValue(null);
				}
			});
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						if(customerCombo.getValue() != null && !customerCombo.getValue().toString().equals("")){
							...
							inquiryList=dao.getSalesInquiryModelCustomerList(getOfficeID(), (Long)customerCombo.getValue(), idList);
							if(inquiryList.size()>0){
								SCollectionContainer bic=SCollectionContainer.setList(inquiryList, "id");
								salesInquiryOptions.setContainerDataSource(bic);
								salesInquiryOptions.setItemCaptionPropertyId("inquiry_no");
								getUI().getCurrent().addWindow(salesInquiryWindow);
							}
							else{
								SNotification.show("No sales Inquiry Available",Type.WARNING_MESSAGE);
							}
						}
						else
			...
			addInquiryButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (salesInquiryOptions.getValue() != null && ((Set<Long>) salesInquiryOptions.getValue()).size() > 0) {
							table.setVisibleColumns(allHeaders);
							Set<Long> salesInquiries = (Set<Long>) salesInquiryOptions.getValue();
							List inquiryList=new ArrayList();
							inquiryList=dao.getAllDataFromSalesInquiry(salesInquiries);
							Iterator itr = inquiryList.iterator();
							while (itr.hasNext()) {
								SalesQuotationBean bean = (SalesQuotationBean) itr.next();
							...
							getUI().removeWindow(salesInquiryWindow);
							calculateTotals();
							calculateNetPrice();
							table.setVisibleColumns(requiredHeaders);
						}
...
//								}
				...
				public void buttonClick(ClickEvent event) {
					try {
						if(isValid()){
							...
							QuotationModel mdl=new QuotationModel();
							List<QuotationDetailsModel> itemsList = new ArrayList<QuotationDetailsModel>();
							Iterator it = table.getItemIds().iterator();
							while (it.hasNext()) {
								QuotationDetailsModel det=new QuotationDetailsModel();
								Item item = table.getItem(it.next());
							...
							if(savable){
								mdl.setQuotation_no(getNextSequence("Quotation Number", getLoginID(), getOfficeID(), CommonUtil.getSQLDateFromUtilDate(dateField.getValue())));
								...
								saveActivity(getOptionId(),
										"sales Quotation Created. Quotation No : "
												+ mdl.getQuotation_no()+ ", customer : "
			...
			salesQuotationCombo.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						if(salesQuotationCombo.getValue()!=null && !salesQuotationCombo.getValue().toString().equals("0")){
							table.setVisibleColumns(allHeaders);
							QuotationModel mdl=dao.getQuotationModel((Long)salesQuotationCombo.getValue());
					...
					try {
						if(isValid()){
							if(salesQuotationCombo.getValue()!=null && !salesQuotationCombo.getValue().toString().equals("0")){
								...
								Iterator it = table.getItemIds().iterator();
								while (it.hasNext()) {
									QuotationDetailsModel det=null;
									Item item = table.getItem(it.next());
									long id=(Long) item.getItemProperty(TBC_ID).getValue();
									if(id!=0)
										det=dao.getQuotationDetailsModel(id);
									if(det==null)
										det=new QuotationDetailsModel();
									double qty=(Double) item.getItemProperty(TBC_QTY).getValue();
									if(qty<=0){
										savable=false;
										break;
								...
								if(savable){
									...
									saveActivity(getOptionId(),
											"sales Quotation Updated. Quotation No : "
													+ mdl.getQuotation_no()+ ", customer : "
													+ customerCombo.getItemCaption(customerCombo.getValue())
								...
								if (dialog.isConfirmed()) {
									try {
										...
										if(mdl.getLock_count()>0){
											...
											saveActivity(getOptionId()," sales Quotation deleted. Quotation No : "
														+ salesQuotationCombo.getItemCaption(salesQuotationCombo.getValue())
														+ ", customer : "+customerCombo.getItemCaption(customerCombo.getValue()),
														(Long)salesQuotationCombo.getValue());
											loadOptions(0);
										}
								...
								if (dialog.isConfirmed()) {
									try {
										...
										if(mdl.getLock_count()>0){
											...
											dao.cancel(mdl);
											Notification.show(getPropertyName("deleted_success"),Type.WARNING_MESSAGE);
											saveActivity(getOptionId()," sales Quotation Cancelled. Quotation No : "
														+ salesQuotationCombo.getItemCaption(salesQuotationCombo.getValue())
														+ ", customer : "+customerCombo.getItemCaption(customerCombo.getValue()),
														(Long)salesQuotationCombo.getValue());
											loadOptions(0);
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if(salesQuotationCombo.getValue()!=null && !salesQuotationCombo.getValue().toString().equals("0")){
							...
							map.put("REPORT_TITLE_LABEL", getPropertyName("sales_quotation"));
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("sales_quotation"));
...
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("sales_quotation"));
								...
								if(configModel!=null){
									if(file.exists())
										mail.sendMailWithFromAddress(mailTo, 
																	"You have a new Sales Quotation from "+ledger.getName()+" Bill No. "+mdl.getQuotation_no(),
																	"Sales Quotation",
																	file, 
																	getUserID());
	...
	public void loadOptions(long id) {
		List list = new ArrayList();
		try {
			list.add(new QuotationModel(0, "----Create New-----"));
			list.addAll(dao.getQuotationModelList(getOfficeID()));
			SCollectionContainer bic = SCollectionContainer.setList(list, "id");
			salesQuotationCombo.setContainerDataSource(bic);
			salesQuotationCombo.setItemCaptionPropertyId("quotation_no");
			salesQuotationCombo.setValue(id);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public SComboField getsalesOrderNumberList() {
		return salesQuotationCombo;
	}
	
	

	
	public void setsalesOrderNumberList(SComboField salesQuotationCombo) {
		this.salesQuotationCombo = salesQuotationCombo;
	}
