...
import com.inventory.purchase.bean.PurchaseQuotationBean;
import com.inventory.purchase.dao.PurchaseQuotationDao;
...
public class PurchaseQuotationUI extends SparkLogic {
	...
	static String TBC_INQUIRY_ID = "Inquiry Id";
	static String TBC_INQUIRY_CHILD_ID = "Inquiry Child Id";

	PurchaseQuotationDao dao;
	SComboField purchaseQuotationCombo;
	...
	SButton addInquiryButton;
	SOptionGroup purchaseInquiryOptions;	
	SWindow purchaseInquiryWindow;
	...
	public SPanel getGUI() {
		...
		allHeaders = new String[] { TBC_SN, TBC_ID, TBC_EDITABLE, TBC_ITEM_ID, TBC_ITEM_CODE, TBC_ITEM_NAME,
				TBC_QTY, TBC_CONVERTION_QTY, TBC_QTY_IN_BASIC_UNIT, TBC_UNIT_ID, TBC_UNIT, TBC_UNIT_PRICE, TBC_CID, TBC_CURRENCY, TBC_CONV_RATE, 
				TBC_NET_PRICE, TBC_INQUIRY_ID, TBC_INQUIRY_CHILD_ID };
		...
		try {
			...
			purchaseQuotationCombo = new SComboField(null, 150, null, "id","ref_no", false, getPropertyName("create_new"));
			...
			salLisrLay.addComponent(purchaseQuotationCombo);
			...
			masterDetailsGrid.addComponent(new SLabel("Quotation No"), 1, 0);
...
//					itmDao.getAllActiveItemsWithAppendingItemCode(getOfficeID()), "id", "name",true,getPropertyName("select"));
			...
			table.addContainerProperty(TBC_INQUIRY_ID, Long.class, null,TBC_INQUIRY_ID, null, Align.CENTER);
			table.addContainerProperty(TBC_INQUIRY_CHILD_ID, Long.class, null,TBC_INQUIRY_CHILD_ID, null, Align.CENTER);
			...
			importButton = new SButton(null, "Import From Purchase Inquiry");
			importButton.setStyleName("importButtonStyle");
			addInquiryButton = new SButton(null,"Import Inquiries");
			addInquiryButton.setStyleName("importButtonStyle");
			purchaseInquiryOptions = new SOptionGroup("Select Purchase Inquiry : ", 300, null, "id", "name", true);
			purchaseInquiryWindow = new SWindow("Purchase Inquiry", 650, 250);
			purchaseInquiryWindow.center();
			purchaseInquiryWindow.setResizable(false);
			purchaseInquiryWindow.setModal(true);
			
			SFormLayout popUpLayout = new SFormLayout();
			popUpLayout.setSpacing(true);
			popUpLayout.setMargin(true);
			popUpLayout.addComponent(purchaseInquiryOptions);
			popUpLayout.addComponent(addInquiryButton);
			purchaseInquiryWindow.setContent(popUpLayout);
			...
			ClickListener confirmListener=new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(event.getButton().getId().equals("1")) {
						try {
							saveReportedIssue(getOptionId(),confirmBox.getComments(),(Long)purchaseQuotationCombo.getValue(),confirmBox.getUserID());
			...
			ClickListener clickListnr=new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(event.getButton().getId().equals(windowNotif.SAVE_SESSION)) {
						if(purchaseQuotationCombo.getValue()!=null && !purchaseQuotationCombo.getValue().toString().equals("0")) {
							saveSessionActivity(getOptionId(), (Long)purchaseQuotationCombo.getValue(),
									"Purchase Order : No. "+purchaseQuotationCombo.getItemCaption(purchaseQuotationCombo.getValue()));
							Notification.show("Success",
									"Session Saved Successfully..!",
									Type.WARNING_MESSAGE);
						}
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						while (it.hasNext()) {
							...
							long id=toLong(item.getItemProperty(TBC_INQUIRY_ID).getValue().toString());
							if(id!=0)
								idList.add(obj);
						}
						if(idList.size()>0){
			...
			createNewButton.addClickListener(new ClickListener() {
				@Override
				public void buttonClick(ClickEvent event) {
					purchaseQuotationCombo.setValue((long) 0);
					purchaseQuotationCombo.setValue(null);
				}
			});
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						if(supplierCombo.getValue() != null && !supplierCombo.getValue().toString().equals("")){
							...
							inquiryList=dao.getPurchaseInquiryModelSupplierList(getOfficeID(), (Long)supplierCombo.getValue(), idList);
							if(inquiryList.size()>0){
								SCollectionContainer bic=SCollectionContainer.setList(inquiryList, "id");
								purchaseInquiryOptions.setContainerDataSource(bic);
								purchaseInquiryOptions.setItemCaptionPropertyId("inquiry_no");
								getUI().getCurrent().addWindow(purchaseInquiryWindow);
							}
							else{
								SNotification.show("No Purchase Inquiry Available",Type.WARNING_MESSAGE);
							}
						}
						else
			...
			addInquiryButton.addClickListener(new ClickListener() {
				
				@Override
				public void buttonClick(ClickEvent event) {
					try {
						if (purchaseInquiryOptions.getValue() != null && ((Set<Long>) purchaseInquiryOptions.getValue()).size() > 0) {
							table.setVisibleColumns(allHeaders);
							Set<Long> purchaseInquiries = (Set<Long>) purchaseInquiryOptions.getValue();
							List inquiryList=new ArrayList();
							inquiryList=dao.getAllDataFromPurchaseInquiry(purchaseInquiries);
							Iterator itr = inquiryList.iterator();
							while (itr.hasNext()) {
								PurchaseQuotationBean bean = (PurchaseQuotationBean) itr.next();
							...
							getUI().removeWindow(purchaseInquiryWindow);
							calculateTotals();
							calculateNetPrice();
							table.setVisibleColumns(requiredHeaders);
						}
					...
					try {
						...
						if (selectedItems != null && selectedItems.size() == 1) {
							...
							if(!(Boolean)item.getItemProperty(TBC_EDITABLE).getValue()){
								...
								updateItemButton.setVisible(false);
								setRequiredError(itemCombo, "Order Created On This Quotation. Cannot Edit or Delete", true);
							}
							else{
								addItemButton.setVisible(false);
				...
				public void buttonClick(ClickEvent event) {
					try {
						if(isValid()){
							...
							PurchaseQuotationModel mdl=new PurchaseQuotationModel();
							List<PurchaseQuotationDetailsModel> itemsList = new ArrayList<PurchaseQuotationDetailsModel>();
							Iterator it = table.getItemIds().iterator();
							while (it.hasNext()) {
								PurchaseQuotationDetailsModel det=new PurchaseQuotationDetailsModel();
								Item item = table.getItem(it.next());
								...
								det.setInquiry_id((Long) item.getItemProperty(TBC_INQUIRY_ID).getValue());
								det.setInquiry_child_id((Long) item.getItemProperty(TBC_INQUIRY_CHILD_ID).getValue());
								itemsList.add(det);
							}
							if(savable){
								mdl.setQuotation_no(getNextSequence("Purchase Quotation No", getLoginID(), getOfficeID(), CommonUtil.getSQLDateFromUtilDate(dateField.getValue())));
								...
								saveActivity(getOptionId(),
										"Purchase Quotation Created. Quotation No : "
												+ mdl.getQuotation_no()+ ", Supplier : "
			...
			purchaseQuotationCombo.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						if(purchaseQuotationCombo.getValue()!=null && !purchaseQuotationCombo.getValue().toString().equals("0")){
							table.setVisibleColumns(allHeaders);
							PurchaseQuotationModel mdl=dao.getPurchaseQuotationModel((Long)purchaseQuotationCombo.getValue());
							...
							while (itr.hasNext()) {
								...
								table.addItem(new Object[]{
										...
										det.getInquiry_id(),
										det.getInquiry_child_id()},table.getItemIds().size()+1);
							}
							calculateTotals();
							table.setVisibleColumns(requiredHeaders);
					...
					try {
						if(isValid()){
							if(purchaseQuotationCombo.getValue()!=null && !purchaseQuotationCombo.getValue().toString().equals("0")){
								...
								Iterator it = table.getItemIds().iterator();
								while (it.hasNext()) {
									PurchaseQuotationDetailsModel det=null;
									Item item = table.getItem(it.next());
									long id=(Long) item.getItemProperty(TBC_ID).getValue();
									if(id!=0)
										det=dao.getPurchaseQuotationDetailsModel(id);
									if(det==null)
										det=new PurchaseQuotationDetailsModel();
									double qty=(Double) item.getItemProperty(TBC_QTY).getValue();
									if(qty<=0){
										savable=false;
										break;
								...
								if(savable){
									...
									saveActivity(getOptionId(),
											"Purchase Quotation Updated. Quotation No : "
													+ mdl.getQuotation_no()+ ", Supplier : "
													+ supplierCombo.getItemCaption(supplierCombo.getValue())
								...
								if (dialog.isConfirmed()) {
									try {
										...
										if(mdl.getLock_count()>0){
											Notification.show("Order Created On This Quotation. Cannot Delete",Type.ERROR_MESSAGE);
										}
										else{
											dao.delete(mdl);
											Notification.show(getPropertyName("deleted_success"),Type.WARNING_MESSAGE);
											saveActivity(getOptionId()," Purchase Quotation deleted. Quotation No : "
														+ purchaseQuotationCombo.getItemCaption(purchaseQuotationCombo.getValue())
														+ ", Supplier : "+supplierCombo.getItemCaption(supplierCombo.getValue()),
														(Long)purchaseQuotationCombo.getValue());
											loadOptions(0);
										}
							...
							public void onClose(ConfirmDialog dialog) {
								if (dialog.isConfirmed()) {
									try {
										PurchaseQuotationModel mdl=dao.getPurchaseQuotationModel((Long)purchaseQuotationCombo.getValue());
										if(mdl.getLock_count()>0){
											Notification.show("Order Created On This Quotation. Cannot Cancel",Type.ERROR_MESSAGE);
										}
										else{
											dao.cancel(mdl);
											Notification.show(getPropertyName("deleted_success"),Type.WARNING_MESSAGE);
											saveActivity(getOptionId()," Purchase Quotation Cancelled. Quotation No : "
														+ purchaseQuotationCombo.getItemCaption(purchaseQuotationCombo.getValue())
														+ ", Supplier : "+supplierCombo.getItemCaption(supplierCombo.getValue()),
														(Long)purchaseQuotationCombo.getValue());
											loadOptions(0);
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if(purchaseQuotationCombo.getValue()!=null && !purchaseQuotationCombo.getValue().toString().equals("0")){
							...
							map.put("REPORT_TITLE_LABEL", getPropertyName("purchase_quotation"));
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("purchase_quotation"));
							...
							map.put("BILL_NO", mdl.getQuotation_no());
							...
							report.setReportFileName("Purchase Quotation Print");
...
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("purchase_quotation"));
								...
								if(configModel!=null){
									if(file.exists())
										mail.sendMailWithFromAddress(mailTo, 
																	"You have a new Purchase Quotation from "+ledger.getName()+" Bill No. "+mdl.getQuotation_no(),
																	"Purchase Quotation",
																	file, 
																	getUserID());
	...
	public void loadOptions(long id) {
		List list = new ArrayList();
		try {
			list.add(new PurchaseQuotationModel(0, "----Create New-----"));
			list.addAll(dao.getPurchaseQuotationModelList(getOfficeID()));
			SCollectionContainer bic = SCollectionContainer.setList(list, "id");
			purchaseQuotationCombo.setContainerDataSource(bic);
			purchaseQuotationCombo.setItemCaptionPropertyId("quotation_no");
			purchaseQuotationCombo.setValue(id);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public SComboField getPurchaseOrderNumberList() {
		return purchaseQuotationCombo;
	}
	
	

	
	public void setPurchaseOrderNumberList(SComboField purchaseQuotationCombo) {
		this.purchaseQuotationCombo = purchaseQuotationCombo;
	}
