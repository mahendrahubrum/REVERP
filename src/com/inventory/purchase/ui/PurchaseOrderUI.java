	...
	public SPanel getGUI() {
		...
		try {
			...
			if (settings.isSHOW_SUPPLIER_SPECIFIC_ITEM_IN_PURCHASE()) {
				itemCombo = new SComboField(getPropertyName("item"), 100, null, "id", "name", true,
						getPropertyName("select"));
			} else {
			...
			addShortcutListener(new ShortcutListener("Save", ShortcutAction.KeyCode.F4, null) {
				@Override
						...
						if (previousDate.getTime() != dateField.getValue().getTime()) {
							...
							if ((Long) netAmountField.currencySelect.getValue() != getCurrencyID()) {
								ConfirmDialog.show(getUI().getCurrent().getCurrent(),
										"Update Currency Rate Accordingly.", new ConfirmDialog.Listener() {
					...
					if (event.getButton().getId().equals("1")) {
						try {
							saveReportedIssue(getOptionId(), confirmBox.getComments(),
									(Long) purchaseOrderCombo.getValue(), confirmBox.getUserID());
			...
			ClickListener clickListnr = new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if (event.getButton().getId().equals(windowNotif.SAVE_SESSION)) {
						if (purchaseOrderCombo.getValue() != null
								&& !purchaseOrderCombo.getValue().toString().equals("0")) {
			...
			table.addShortcutListener(new ShortcutListener("Delete Item", ShortcutAction.KeyCode.N,
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						if (supplierCombo.getValue() != null && !supplierCombo.getValue().toString().equals("")) {
							...
							quotationList = dao.getPurchaseQuotationModelSupplierList(getOfficeID(),
			...
			addQuotationButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (purchaseQuotationOptions.getValue() != null
								&& ((Set<Long>) purchaseQuotationOptions.getValue()).size() > 0) {
							...
							while (itr.hasNext()) {
								...
								table.addItem(new Object[] { table.getItemIds().size() + 1, (long) 0, true,
										...
										currency, roundNumber(bean.getDet().getConversionRate()),
										roundNumber(bean.getDet().getUnit_price() * bean.getDet().getQunatity()),
										bean.getId(), bean.getDet().getId() }, table.getItemIds().size() + 1);
							}
					...
					try {
						...
						if (selectedItems != null && selectedItems.size() == 1) {
							...
							if (!(Boolean) item.getItemProperty(TBC_EDITABLE).getValue()) {
								...
								updateItemButton.setVisible(false);
								setRequiredError(itemCombo, "Goods Received On This Order. Cannot Edit or Delete",
										true);
							} else {
								addItemButton.setVisible(false);
					...
					try {
						if (isValid()) {
							...
							if (savable) {
								...
								mdl.setOrder_no(getNextSequence("Purchase Order Id", getLoginID(), getOfficeID(),
								...
								saveActivity(getOptionId(),
										"Purchase Order Created. Order No : " + mdl.getOrder_no() + ", Supplier : "
												+ supplierCombo.getItemCaption(supplierCombo.getValue())
												+ ", Approximate Amount : " + mdl.getAmount(),
										id);
							} else
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						if (isValid()) {
							if (purchaseOrderCombo.getValue() != null
									&& !purchaseOrderCombo.getValue().toString().equals("0")) {
								PurchaseOrderModel mdl = dao
										.getPurchaseOrderModel((Long) purchaseOrderCombo.getValue());
								List<PurchaseOrderDetailsModel> itemsList = new ArrayList<PurchaseOrderDetailsModel>();
								boolean savable = true;
								Iterator it = table.getItemIds().iterator();
								while (it.hasNext()) {
...
//									mdl.setOffice(new S_OfficeModel(getOfficeID()));
									...
									saveActivity(getOptionId(),
											"Purchase Order Updated. Order No : " + mdl.getOrder_no() + ", Supplier : "
													+ supplierCombo.getItemCaption(supplierCombo.getValue())
													+ ", Approximate Amount : " + mdl.getAmount(),
											mdl.getId());
								} else
								...
								if (dialog.isConfirmed()) {
									try {
										...
										if (mdl.getLock_count() > 0) {
											Notification.show("Purchase Created On This Order. Cannot Delete",
													Type.ERROR_MESSAGE);
										} else {
											dao.delete(mdl);
											Notification.show(getPropertyName("deleted_success"), Type.WARNING_MESSAGE);
											saveActivity(getOptionId(),
													" Purchase Order deleted. Order No : "
															+ purchaseOrderCombo.getItemCaption(
																	purchaseOrderCombo.getValue())
															+ ", Supplier : "
															+ supplierCombo.getItemCaption(supplierCombo.getValue()),
								...
								if (dialog.isConfirmed()) {
									try {
										PurchaseOrderModel mdl = dao
												.getPurchaseOrderModel((Long) purchaseOrderCombo.getValue());
										if (mdl.getLock_count() > 0) {
											Notification.show("Purchase Created On This Order. Cannot Cancel",
													Type.ERROR_MESSAGE);
										} else {
											dao.cancel(mdl);
											Notification.show(getPropertyName("deleted_success"), Type.WARNING_MESSAGE);
											saveActivity(getOptionId(),
													" Purchase Order Cancelled. Order No : "
															+ purchaseOrderCombo.getItemCaption(
																	purchaseOrderCombo.getValue())
															+ ", Supplier : "
															+ supplierCombo.getItemCaption(supplierCombo.getValue()),
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if (purchaseOrderCombo.getValue() != null
								&& !purchaseOrderCombo.getValue().toString().equals("0")) {
							PurchaseOrderModel mdl = dao.getPurchaseOrderModel((Long) purchaseOrderCombo.getValue());
							SupplierModel supplier = supDao
									.getSupplierFromLedger(toLong(supplierCombo.getValue().toString()));
							String address = "";
							if (supplier != null) {

								address = new AddressBusiness().getAddressString(supplier.getAddress().getId());
...
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("purchase_order"));
							...
							map.put("SHIP_TO", getPropertyName("ship_to"));
							S_OfficeModel office = new OfficeDao().getOffice(mdl.getOffice().getId());
							map.put("OFFICE_NAME", office.getName());
							map.put("OFFICE_ADDRESS", new AddressBusiness().createAddressAsString(office.getAddress()));
							map.put("ADDRESS_LABEL", getPropertyName("address"));
//							map.put("OFFICE_VAT_NUMBER", office.getVatNumber());
							map.put("DELIVERY_TERMS_MENTIONED", getPropertyName("delivery_terms_as_mentioned"));
							map.put("DELIVERY_DATE_MENTIONED", getPropertyName("delivery_date_as_mentioned"));
							map.put("PAYMENT_TERMS_MENTIONED", getPropertyName("payment_terms_as_mentioned"));
							map.put("FOR_LABEL", "FOR");
							map.put("TO_LABEL", "TO");
							map.put("THE_FOLLOWING_CONSTITUTES", getPropertyName("the_following_constitutes"));
							map.put("SIGNATURE", getPropertyName("signature"));
							map.put("NAME", getPropertyName("name"));
							map.put("POSITION", getPropertyName("position"));
							map.put("DATE", getPropertyName("date"));
							map.put("VAT_NO_LABEL_SUPP", getPropertyName("vat_num_label"));
...
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("purchase_quotation"));
							...
							if (toAddr != null) {
								...
								if (configModel != null) {
									if (file.exists())
										mail.sendMailWithFromAddress(
												mailTo, "You have a new Purchase Order from " + ledger.getName()
														+ " Bill No. " + mdl.getOrder_no(),
												"Purchase Order", file, getUserID());
