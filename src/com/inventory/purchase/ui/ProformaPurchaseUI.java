...
public class ProformaPurchaseUI extends SparkLogic {
	...
	public SPanel getGUI() {
		...
		/*
		 * static String TBC_DISCOUNT = "Discount"; static String TBC_TAX_ID = "Tax Id";
		 * static String TBC_TAX_PERCENTAGE = "Tax Percentage"; static String
		 * TBC_TAX_AMT = "Tax Amount"; static String TBC_CESS = "Cess";
		 */

		allHeaders = new String[] { TBC_SN, TBC_ID, TBC_EDITABLE, TBC_ITEM_ID, TBC_ITEM_CODE, TBC_ITEM_NAME, TBC_QTY,
				TBC_CONVERTION_QTY, TBC_QTY_IN_BASIC_UNIT, TBC_QTY_ORDERED, TBC_BALANCE, TBC_UNIT_ID, TBC_UNIT,
				TBC_UNIT_PRICE, TBC_CID, TBC_CURRENCY, TBC_CONV_RATE, TBC_DISCOUNT, TBC_TAX_ID, TBC_TAX_PERCENTAGE,
				TBC_TAX_AMOUNT, TBC_CESS, TBC_NET_PRICE, TBC_MANUFACTURING_DATE_ID, TBC_MANUFACTURING_DATE,
				TBC_EXPIRY_DATE_ID, TBC_EXPIRY_DATE, TBC_GRADE_ID, TBC_GRADE, TBC_LOCATION_ID, TBC_LOCATION,
				TBC_BARCODE, TBC_BATCH_ID, TBC_STOCK_ID, TBC_ORDER_ID, TBC_ORDER_CHILD_ID, TBC_GRN_ID,
				TBC_GRN_CHILD_ID };

		requiredHeaders = new String[] { TBC_SN, TBC_ITEM_CODE, TBC_ITEM_NAME, TBC_QTY,
				/* TBC_QTY_ORDERED, TBC_BALANCE, */ TBC_UNIT, TBC_UNIT_PRICE, TBC_CURRENCY, TBC_DISCOUNT,
				TBC_TAX_AMOUNT, TBC_CESS, TBC_MANUFACTURING_DATE, TBC_EXPIRY_DATE, TBC_GRADE, TBC_LOCATION, TBC_BARCODE,
				TBC_NET_PRICE };
...
//		itemLayout.setSizeFull();
		...
		try {
			...
			proformaPurchaseCombo = new SComboField(null, 150, null, "id", "ref_no", false,
					getPropertyName("create_new"));
			referenceNoField = new STextField(null, 150);
			dateField = new SDateField(null, 100, getDateFormat());
			supplierCombo = new SComboField(null, 150, new LedgerDao().getAllSuppliers(getOfficeID()), "id", "name",
			...
			departmentCombo = new SComboField(null, 100, departmentList, "id", "name", false,
			...
			purchaseAccountCombo = new SComboField("Purchase Account", 150,
					new LedgerDao().getAllActiveGeneralLedgerOnly(getOfficeID()), "id", "name");
			...
			locationMasterCombo = new SComboField(null, 100, locationList, "id", "name", true,
					getPropertyName("select"));
			...
			itemCombo = new SComboField(getPropertyName("item"), 100,
					itmDao.getAllActiveItemsWithAppendingItemCode(getOfficeID()), "id", "name", true,
					getPropertyName("select"));
			...
			locationCombo = new SComboField("Location", 100, locationList, "id", "name", true,
					getPropertyName("select"));
			...
			taxSelect = new SNativeSelect(getPropertyName("tax"), 75,
					taxDao.getAllActiveTaxesFromType(getOfficeID(), SConstants.tax.PURCHASE_TAX), "id", "name");
			...
			table.addContainerProperty(TBC_ITEM_CODE, String.class, null, getPropertyName("item_code"), null,
					Align.CENTER);
			...
			table.addContainerProperty(TBC_NET_PRICE, Double.class, null, getPropertyName("net_price"), null,
					Align.LEFT);
			table.addContainerProperty(TBC_MANUFACTURING_DATE_ID, Date.class, null, TBC_MANUFACTURING_DATE_ID, null,
					Align.LEFT);
			table.addContainerProperty(TBC_MANUFACTURING_DATE, String.class, null, TBC_MANUFACTURING_DATE, null,
					Align.LEFT);
...
//			bottomGrid.setColumnExpandRatio(7, 0.5f);
					...
					if (dateField.getValue() != null) {
						if (previousDate.getTime() != dateField.getValue().getTime()) {
							...
							if ((Long) netAmountField.currencySelect.getValue() != getCurrencyID()) {
								ConfirmDialog.show(getUI().getCurrent().getCurrent(),
										"Are You Sure ? Update Currency Rate Accordingly.",
										new ConfirmDialog.Listener() {
											public void onClose(ConfirmDialog dialog) {
												...
												if (expense < 0) {
													...
													expenseField.setNewValue(
															roundNumber(expensePanel.getDebitAmount()) + "");
												}
											}
										});
							}
					...
					if (event.getButton().getId().equals("1")) {
						try {
							saveReportedIssue(getOptionId(), confirmBox.getComments(),
									(Long) proformaPurchaseCombo.getValue(), confirmBox.getUserID());
			...
			ClickListener clickListnr = new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if (event.getButton().getId().equals(windowNotif.SAVE_SESSION)) {
						if (proformaPurchaseCombo.getValue() != null
								&& !proformaPurchaseCombo.getValue().toString().equals("0")) {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						if (netAmountField.currencySelect.getValue() != null) {
							...
							netAmountField.setNewValue(
									roundNumber(convertedField.getValue() * netAmountField.getConversionRate()));
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			table.addShortcutListener(new ShortcutListener("Clear entereded and edited data and Add new",
			...
			expenseField.addValueChangeListener(new ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						convertedField
								.setNewValue(roundNumber(toDouble(table.getColumnFooter(TBC_NET_PRICE)) + expense));
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					try {
						...
						if (supplierCombo.getValue() != null) {
							...
							supplierCombo.setDescription("<h1><i>Current Balance</i> : "
									+ comDao.getLedgerCurrentBalance((Long) supplierCombo.getValue()) + "</h1>");
							...
							contactField.setNewValue(
									supObj.getAddress().getPhone() + ", " + supObj.getAddress().getMobile());
						} else {
						...
						if (paymentCreditRadio.getValue() != null) {
							if ((Long) paymentCreditRadio.getValue() == SConstants.paymentMode.PAYMENT) {
								...
								list = new LedgerDao().getAllActiveLedgerNamesExcluding(getOfficeID(),
										SConstants.LEDGER_ADDED_INDIRECTLY, settings.getCASH_GROUP());
								SCollectionContainer bic = SCollectionContainer.setList(list, "id");
							...
							if (cashChequeRadio.isVisible()) {
								if ((Long) cashChequeRadio.getValue() == SConstants.paymentMode.CASH) {
									...
									list = new LedgerDao().getAllActiveLedgerNamesOfGroup(getOfficeID(),
											SConstants.LEDGER_ADDED_DIRECTLY, settings.getCASH_GROUP());
									SCollectionContainer bic = SCollectionContainer.setList(list, "id");
									purchaseAccountCombo.setContainerDataSource(bic);
									purchaseAccountCombo.setItemCaptionPropertyId("name");
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						if (supplierCombo.getValue() != null && !supplierCombo.getValue().toString().equals("")) {
							...
							orderList = dao.getPurchaseOrderModelSupplierList(getOfficeID(),
									(Long) supplierCombo.getValue(), idList);
							if (orderList.size() > 0) {
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						if (supplierCombo.getValue() != null && !supplierCombo.getValue().toString().equals("")) {
							...
							grnList = dao.getPurchaseGRNModelSupplierList(getOfficeID(),
									(Long) supplierCombo.getValue(), idList);
							if (grnList.size() > 0) {
			...
			addOrderButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (purchaseOrderOptions.getValue() != null
								&& ((Set<Long>) purchaseOrderOptions.getValue()).size() > 0) {
							...
							while (itr.hasNext()) {
								...
								String currency = new CurrencyManagementDao()
										.getselecteditem(bean.getDet().getCurrencyId()).getCode();
								...
								table.addItem(new Object[] { table.getItemIds().size() + 1, (long) 0, true,
										bean.getDet().getItem().getId(), bean.getDet().getItem().getItem_code(),
										bean.getDet().getItem().getName(), (double) 0.0, (double) 1, (double) 0,
										roundNumber(bean.getDet().getQty_in_basic_unit()),
										roundNumber(bean.getDet().getQty_in_basic_unit()
												- bean.getDet().getQuantity_received()),
										bean.getDet().getUnit().getId(), bean.getDet().getUnit().getSymbol(),
										roundNumber(bean.getDet().getUnit_price()), bean.getDet().getCurrencyId(),
										currency, roundNumber(bean.getDet().getConversionRate()), roundNumber(discount),
										taxId, roundNumber(taxPer), roundNumber(taxAmount), roundNumber(cess),
										roundNumber(
												bean.getDet().getUnit_price() * 0 / bean.getDet().getConversionRate()
														+ taxAmount + cess - discount),
										getWorkingDate(), CommonUtil.formatDateToDDMMYYYY(getWorkingDate()),
										getWorkingDate(), CommonUtil.formatDateToDDMMYYYY(getWorkingDate()), (long) 0,
										"None", (long) 0, "None", "", (long) 0, (long) 0, bean.getId(),
										bean.getDet().getId(), (long) 0, (long) 0, }, table.getItemIds().size() + 1);
							}
			...
			addGRNButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (purchaseGRNOptions.getValue() != null
								&& ((Set<Long>) purchaseGRNOptions.getValue()).size() > 0) {
							table.setVisibleColumns(allHeaders);
							...
							while (itr.hasNext()) {
								...
								table.addItem(new Object[] { table.getItemIds().size() + 1, (long) 0, false,
										bean.getDet().getItem().getId(), bean.getDet().getItem().getItem_code(),
										bean.getDet().getItem().getName(), roundNumber(bean.getDet().getQunatity()),
										...
										roundNumber(bean.getDet().getUnit_price() * bean.getDet().getQunatity()
												/ bean.getDet().getConversionRate() + taxAmount + cess - discount),
										getWorkingDate(), CommonUtil.formatDateToDDMMYYYY(getWorkingDate()),
										getWorkingDate(), CommonUtil.formatDateToDDMMYYYY(getWorkingDate()),
										...
										bean.getDet().getBatch_id(), bean.getDet().getStock_id(),
										bean.getDet().getOrder_id(), bean.getDet().getOrder_child_id(), bean.getId(),
					...
					try {
						if (itemCombo.getValue() != null) {
							...
							SCollectionContainer bic = SCollectionContainer
									.setList(comDao.getAllItemUnitDetails(itm.getId()), "id");
					...
					try {
						if (unitSelect.getValue() != null) {
							...
							if (itemCombo.getValue() != null) {
								...
								unitPriceField.setNewValue(
										comDao.getItemCurrency(itm.getId(), (Long) unitSelect.getValue(), (long) 0),
										roundNumber(comDao.getItemPrice(itm.getId(), (Long) unitSelect.getValue(),
												(long) 0)));
								if (quantityField.getValue() != null
										&& !quantityField.getValue().toString().equals("0")) {
									convertedQuantityField
											.setNewValue(asString(Double.parseDouble(quantityField.getValue())
													* Double.parseDouble(convertionQuantityField.getValue())));
									netPriceField.setNewValue(
											asString(unitPriceField.getValue() * toDouble(quantityField.getValue())));
								}
							}
						} else
			...
			convertionQuantityField.addValueChangeListener(new Property.ValueChangeListener() {
				public void valueChange(ValueChangeEvent event) {
					try {
						if (convertionQuantityField.getValue().equals("")
								|| toDouble(convertionQuantityField.getValue()) <= 0) {
							convertionQuantityField.setValue("1");
						}
						calculateNetPrice();
					} catch (Exception e) {
					...
					try {
						if (isAddingValid()) {
							...
							table.addItem(new Object[] { table.getItemIds().size() + 1, (long) 0, true,
									...
									roundNumber(conv_rat), roundNumber(conv_rat * qty), (double) 0.0, (double) 0.0,
									unitModel.getId(), unitModel.getSymbol(), roundNumber(amount),
									unitPriceField.getCurrency(), currency,
									roundNumber(unitPriceField.getConversionRate()), roundNumber(discount), taxId,
									roundNumber(taxPer), roundNumber(taxAmount), roundNumber(cess),
									roundNumber(netPrice), manufacturingDate,
									CommonUtil.formatDateToDDMMYYYY(manufacturingDate), expiryDate,
									CommonUtil.formatDateToDDMMYYYY(expiryDate), (Long) gradeCombo.getValue(),
									...
									barcodeField.getValue(), (long) 0, (long) 0, (long) 0, (long) 0, (long) 0,
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						if (selectedItems != null && selectedItems.size() == 1) {
							...
							quantityField
									.setNewValue(roundNumber((Double) item.getItemProperty(TBC_QTY).getValue()) + "");
							...
							manufacturingDateField
									.setNewValue((Date) item.getItemProperty(TBC_MANUFACTURING_DATE_ID).getValue());
							...
							if (grnId == 0) {
								if (!(Boolean) item.getItemProperty(TBC_EDITABLE).getValue()) {
									...
									updateItemButton.setVisible(false);
									setRequiredError(itemCombo, "Goods Returned From This Order. Cannot Edit or Delete",
											true);
								} else {
									addItemButton.setVisible(false);
					...
					try {
						if (isAddingValid()) {
							...
							if (selectedItems != null && selectedItems.size() == 1) {
								...
								ItemModel itemModel = itemDao.getItem((Long) itemCombo.getValue());
								UnitModel unitModel = untDao.getUnit((Long) unitSelect.getValue());
								double requested = toDouble(
										item.getItemProperty(TBC_QTY_ORDERED).getValue().toString());
								double balance = toDouble(item.getItemProperty(TBC_BALANCE).getValue().toString());
								double oldQty = toDouble(
										item.getItemProperty(TBC_QTY_IN_BASIC_UNIT).getValue().toString());
								double qty = toDouble(quantityField.getValue().toString());
								double amount = unitPriceField.getValue();
								String currency = new CurrencyManagementDao()
										.getselecteditem(unitPriceField.getCurrency()).getCode();
								double conv_rat;
								try {
									conv_rat = toDouble(convertionQuantityField.getValue());
								} catch (Exception e) {
								...
								item.getItemProperty(TBC_MANUFACTURING_DATE)
										.setValue(CommonUtil.formatDateToDDMMYYYY(manufacturingDate));
								item.getItemProperty(TBC_EXPIRY_DATE_ID).setValue(expiryDate);
								item.getItemProperty(TBC_EXPIRY_DATE)
										.setValue(CommonUtil.formatDateToDDMMYYYY(expiryDate));
								item.getItemProperty(TBC_GRADE_ID).setValue((Long) gradeCombo.getValue());
								item.getItemProperty(TBC_GRADE)
										.setValue(gradeCombo.getItemCaption((Long) gradeCombo.getValue()));
								item.getItemProperty(TBC_LOCATION_ID).setValue((Long) locationCombo.getValue());
								item.getItemProperty(TBC_LOCATION)
										.setValue(locationCombo.getItemCaption((Long) locationCombo.getValue()));
					...
					try {
						if (isValid()) {
							...
							while (it.hasNext()) {
								...
								det.setQty_in_basic_unit(
										roundNumber((Double) item.getItemProperty(TBC_QTY_IN_BASIC_UNIT).getValue()));
								...
								det.setConversionRate(
										roundNumber((Double) item.getItemProperty(TBC_CONV_RATE).getValue()));
								...
								det.setManufacturing_date(CommonUtil.getSQLDateFromUtilDate(
										(Date) item.getItemProperty(TBC_MANUFACTURING_DATE_ID).getValue()));
								det.setExpiry_date(CommonUtil.getSQLDateFromUtilDate(
										(Date) item.getItemProperty(TBC_EXPIRY_DATE_ID).getValue()));
							...
							if (savable) {
								mdl.setProforma_no(getNextSequence("Proforma Purchase No", getLoginID(), getOfficeID(),
								...
								saveActivity(getOptionId(),
										"Proforma Purchase Created. Proforma No : " + mdl.getProforma_no()
												+ ", Supplier : "
												+ supplierCombo.getItemCaption(supplierCombo.getValue()) + ", Amount : "
												+ mdl.getAmount(),
										id);
							} else
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						if (proformaPurchaseCombo.getValue() != null
								&& !proformaPurchaseCombo.getValue().toString().equals("0")) {
							table.setVisibleColumns(allHeaders);
							ProformaPurchaseModel mdl = dao
									.getProformaPurchaseModel((Long) proformaPurchaseCombo.getValue());
							...
							payingAmountField.conversionField
									.setValue("" + roundNumber(mdl.getPaymentConversionRate()));
							...
							while (itr.hasNext()) {
								ProformaPurchaseInventoryDetailsModel det = (ProformaPurchaseInventoryDetailsModel) itr
										.next();
								String currency = new CurrencyManagementDao().getselecteditem(det.getCurrencyId())
										.getCode();
								boolean editable = true;

								if (det.getGrn_id() == 0) {
									if (comDao.isStockBlocked(mdl.getId(), det.getId(),
								...
								if (det.getOrder_child_id() != 0)
									podet = new PurchaseOrderDao()
											.getPurchaseOrderDetailsModel(det.getOrder_child_id());
								if (podet != null) {
								...
								table.addItem(
										new Object[] { table.getItemIds().size() + 1, det.getId(), editable,
												det.getItem().getId(), det.getItem().getItem_code(),
												det.getItem().getName(), roundNumber(det.getQunatity()),
												roundNumber(det.getQty_in_basic_unit() / det.getQunatity()),
												roundNumber(det.getQty_in_basic_unit()), roundNumber(requested),
												roundNumber(balance), det.getUnit().getId(), det.getUnit().getSymbol(),
												roundNumber(det.getUnit_price()), det.getCurrencyId(), currency,
												roundNumber(det.getConversionRate()), roundNumber(det.getDiscount()),
												det.getTax().getId(), roundNumber(det.getTaxPercentage()),
												roundNumber(det.getTaxAmount()), roundNumber(det.getCessAmount()),
												roundNumber((det.getUnit_price() * det.getQunatity())
														+ det.getTaxAmount() + det.getCessAmount() - det.getDiscount()),
												...
												det.getGrade_id(), gradeCombo.getItemCaption(det.getGrade_id()),
												det.getLocation_id(),
												locationCombo.getItemCaption(det.getLocation_id()), det.getBarcode(),
												det.getBatch_id(), det.getStock_id(), det.getOrder_id(),
												det.getOrder_child_id(), det.getGrn_id(), det.getGrn_child_id() },
										table.getItemIds().size() + 1);
							}
							itr = mdl.getProforma_purchase_expense_list().iterator();
							List<ExpenseBean> resultList = new ArrayList<ExpenseBean>();
							while (itr.hasNext()) {
								ProformaPurchaseExpenseDetailsModel det = (ProformaPurchaseExpenseDetailsModel) itr
										.next();
								ExpenseBean bean = new ExpenseBean(det.isClearingAgent(), det.getLedger_id(),
										det.getTransaction_type(), det.getCurrencyId(),
										roundNumber(det.getConversionRate()), roundNumber(det.getAmount()));
								resultList.add(bean);
							}
						...
						if (isValid()) {
							if (proformaPurchaseCombo.getValue() != null
								...
								if (savable) {
									...
									saveActivity(getOptionId(),
											"Proforma Purchase Updated. Proforma No : " + mdl.getProforma_no()
													+ ", Supplier : "
									...
									if (dialog.isConfirmed()) {
										try {
											...
											if (mdl.getLock_count() > 0) {
												Notification.show(
														"Purchase Return Created On This Purchase. Cannot Delete",
														Type.ERROR_MESSAGE);
											} else {
												dao.delete(mdl);
												...
												saveActivity(getOptionId(), "Proforma Purchase deleted. Proforma No : "
														+ proformaPurchaseCombo.getItemCaption(
																proformaPurchaseCombo.getValue())
														+ ", Supplier : "
														+ supplierCombo.getItemCaption(supplierCombo.getValue()),
										...
										try {
											...
											if (mdl.getLock_count() > 0) {
												...
												Notification.show(getPropertyName("deleted_success"),
														Type.WARNING_MESSAGE);
												saveActivity(getOptionId(),
														"Proforma Purchase Cancelled. Proforma No : "
																+ proformaPurchaseCombo.getItemCaption(
																		proformaPurchaseCombo.getValue())
																+ ", Supplier : "
																+ supplierCombo
																		.getItemCaption(supplierCombo.getValue()),
														(Long) proformaPurchaseCombo.getValue());
												loadOptions(0);
											}
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if (proformaPurchaseCombo.getValue() != null
							...
							ProformaPurchaseModel mdl = dao
									.getProformaPurchaseModel((Long) proformaPurchaseCombo.getValue());
							SupplierModel ledger = supDao
									.getSupplierFromLedger(toLong(supplierCombo.getValue().toString()));
							String address = "";
							if (ledger != null) {
...
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("proforma_purchase"));
							...
							map.put("CURRENCY",
									new CurrencyManagementDao().getselecteditem(mdl.getCurrency_id()).getCode());
...
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("proforma_purchase"));
							...
							if (toAddr != null) {
								...
								EmailConfigurationModel configModel = new EmailConfigDao()
										.getEmailConfiguration(getUserID());
								if (configModel != null) {
									if (file.exists())
										mail.sendMailWithFromAddress(mailTo,
												"You have a new Proforma Purchase from " + ledger.getName()
														+ " Bill No. " + mdl.getProforma_no(),
												"Proforma Purchase", file, getUserID());
	...
	public Boolean isValid() {
		...
		if ((Long) netAmountField.currencySelect.getValue() != getCurrencyID() && dateField.getValue() != null) {
			...
			try {
				double value = new CommonMethodsDao().getCurrencyRate(
						CommonUtil.getSQLDateFromUtilDate(dateField.getValue()),
	 ...
	 * reloadConvertionRate() { Iterator itr = table.getItemIds().iterator(); while
	 * (itr.hasNext()) { Item item = table.getItem(itr.next());
	 * unitPriceField.setNewValue((Long)item.getItemProperty(TBC_CID).getValue(),
	 * (Double)item.getItemProperty(TBC_UNIT_PRICE).getValue());
	 * item.getItemProperty(TBC_CONV_RATE).setValue(CommonUtil.roundNumber(
	 * unitPriceField.getConversionRate())); double
	 * netPrice=(unitPriceField.getValue()/unitPriceField.getConversionRate())*(
	 * Double)item.getItemProperty(TBC_QTY).getValue()
	 ...
	 * ); } unitPriceField.setNewValue(getCurrencyID(), 0); calculateTotals(); }
	 */
