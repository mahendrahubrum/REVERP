...
public class BankAccountDepositUI extends SparkLogic {
...
//	DocumentAttach docAttach;
	...
	public SPanel getGUI() {
		...
		cashChequeRadio = new SRadioButton(getPropertyName("payment_type"), 250, SConstants.bank_account.cashChequeCustomerList,"intKey", "value");
		cashChequeRadio.setHorizontal(true);
		
		selectSalesButton=new SButton(null,"Select Sales");
							...
							ledgerCombo.setDescription("<i class='ledger_bal_style'>Current Balance : "
									+ ledgerDao
											.getLedgerCurrentBalance((Long) ledgerCombo
													.getValue()) + "</i>");
							if ((Integer) cashChequeRadio.getValue() == SConstants.bank_account.CUSTOMER) {
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if (isAddingValid()) {
							...
							if((Integer)cashChequeRadio.getValue()==SConstants.bank_account.CHEQUE || (Integer)cashChequeRadio.getValue()==SConstants.bank_account.CUSTOMER)
			...
			saveButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (isValid()) {
							if (paymentCombo.getValue() == null || paymentCombo.getValue().toString().equals("0")) {
								BankAccountDepositModel mdl = new BankAccountDepositModel();
								...
								mdl.setBill_no(getNextSequence("Bank Account Deposit", getLoginID(), getOfficeID(), CommonUtil.getSQLDateFromUtilDate(dateField.getValue())));
								mdl.setActive(true);
								...
								while (itr.hasNext()) {
									...
									BankAccountDepositDetailsModel det=new BankAccountDepositDetailsModel();
								...
								mdl.setBank_account_deposit_list(childList);


								long id = dao.save(mdl,
												tran.getTransaction(SConstants.BANK_ACCOUNT_DEPOSITS,CommonUtil.getSQLDateFromUtilDate(dateField.getValue())),
												invoiceMapList);
								

								saveActivity(getOptionId(), "Bank Account Deposit Saved. Bill No : "
			...
			paymentCombo.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					try {
						...
						resetItems();
						if (paymentCombo.getValue() != null && !paymentCombo.getValue().toString().equals("0")) {
							BankAccountDepositModel objModel = dao.getBankAccountDepositModel((Long) paymentCombo.getValue());
							...
							Iterator it = objModel.getBank_account_deposit_list().iterator();
							while (it.hasNext()) {
								BankAccountDepositDetailsModel det = (BankAccountDepositDetailsModel) it.next();
			...
			updateButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (paymentCombo.getValue() != null && !paymentCombo.getValue().toString().equals("0")) {
							if (isValid()) {
								BankAccountDepositModel mdl = dao.getBankAccountDepositModel((Long) paymentCombo.getValue());
								List<BankAccountDepositDetailsModel> childList=new ArrayList<BankAccountDepositDetailsModel>();
								List<PaymentInvoiceBean> invoiceMapList=new ArrayList<PaymentInvoiceBean>();
								...
								while (itr.hasNext()) {
									...
									boolean isCheque=false;
									Item item = table.getItem(itr.next());
									BankAccountDepositDetailsModel det=null;
									
									if((Long) item.getItemProperty(TBC_ID).getValue()!=0)
										det=dao.getBankAccountDepositDetailsModel((Long) item.getItemProperty(TBC_ID).getValue());
									
									if(det==null)
										det=new BankAccountDepositDetailsModel();
								...
								saveActivity(getOptionId(),"Bank Account Deposit Updated. Bill No : "
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if(paymentCombo.getValue()!=null && !paymentCombo.getValue().toString().equals("0")){
							...
							map.put("REPORT_TITLE_LABEL", getPropertyName("bank_account_deposit"));
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("bank_account_deposit"));
...
//							map.put("NET_PRICE_LABEL", getPropertyName("net_price"));
							...
							report.setReportFileName("Bank Account Deposit Print");
	...
	public void loadData(long id) {
		List list= new ArrayList();
		try {
			list.add(new BankAccountDepositModel(0, "----Create New-----"));
			list.addAll(dao.getBankAccountDepositModelList(getOfficeID()));
