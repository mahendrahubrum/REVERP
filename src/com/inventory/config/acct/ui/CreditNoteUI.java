...
public class CreditNoteUI extends SparkLogic {
...
//		docAttach = new DocumentAttach(SConstants.documentAttach.CHEQUE);
		...
		try {
			...
			creditNoteCombo = new SComboField(null, 200, null, "id",
			...
			masterDetailsGrid.addComponent(new SLabel("Credit Note No", 30), 1, 0);
			SHorizontalLayout salLisrLay = new SHorizontalLayout();
			salLisrLay.addComponent(creditNoteCombo);
			...
			ClickListener confirmListener=new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(event.getButton().getId().equals("1")) {
						try {
							saveReportedIssue(getOptionId(),confirmBox.getComments(),(Long)creditNoteCombo.getValue(),confirmBox.getUserID());
			...
			ClickListener clickListnr=new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(event.getButton().getId().equals(windowNotif.SAVE_SESSION)) {
						if(creditNoteCombo.getValue()!=null && !creditNoteCombo.getValue().toString().equals("0")) {
							saveSessionActivity(getOptionId(), (Long)creditNoteCombo.getValue(),
									"Bank Account Deposit : No. "+creditNoteCombo.getItemCaption(creditNoteCombo.getValue()));
							Notification.show("Success",
									"Session Saved Successfully..!",
									Type.WARNING_MESSAGE);
						}
			...
			createNewButton.addClickListener(new ClickListener() {
				@Override
				public void buttonClick(ClickEvent event) {
					creditNoteCombo.setValue((long) 0);
				}
			});
			...
			table.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					if (table.getValue() != null) {
						...
						if(item.getItemProperty(TBC_BILL_NO).getValue()!=null&&item.getItemProperty(TBC_BILL_NO).getValue().toString().trim().length()>0)
							amountField.currencySelect.setReadOnly(true);
						else
			...
			saveButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (isValid()) {
							if (creditNoteCombo.getValue() == null || creditNoteCombo.getValue().toString().equals("0")) {
								CreditNoteModel mdl = new CreditNoteModel();
								List<CreditNoteDetailsModel> childList=new ArrayList<CreditNoteDetailsModel>();
								...
								mdl.setBill_no(getNextSequence("Credit Note Number", getLoginID(), getOfficeID(), CommonUtil.getSQLDateFromUtilDate(dateField.getValue())));
								...
								while (itr.hasNext()) {
									...
									CreditNoteDetailsModel det=new CreditNoteDetailsModel();
									...
									bean=new PaymentInvoiceBean(SConstants.creditDebitNote.CREDIT,
																type,
								...
								mdl.setCredit_note_list(childList);


								long id = dao.save(mdl,
												tran.getTransaction(SConstants.CREDIT_NOTE,CommonUtil.getSQLDateFromUtilDate(dateField.getValue())),
												invoiceMapList);
								

								saveActivity(getOptionId(), "Credit Note Saved. Bill No : "
			...
			creditNoteCombo.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					try {
						...
						if (creditNoteCombo.getValue() != null && !creditNoteCombo.getValue().toString().equals("0")) {
							...
							CreditNoteModel mdl = dao.getCreditNoteModel((Long) creditNoteCombo.getValue());
							...
							Iterator it = mdl.getCredit_note_list().iterator();
							while (it.hasNext()) {
								CreditNoteDetailsModel det = (CreditNoteDetailsModel) it.next();
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if (isValid()) {
							...
							saveActivity(getOptionId(), "Credit Note Updated. Bill No : "
					...
					if (creditNoteCombo.getValue() != null && !creditNoteCombo.getValue().toString() .equals("0")) {
						ConfirmDialog.show(getUI(), "Are you sure?", new ConfirmDialog.Listener() {
							public void onClose(ConfirmDialog dialog) {
								if (dialog.isConfirmed()) {
									try {
										dao.delete((Long) creditNoteCombo.getValue());
										saveActivity(getOptionId(), "Credit Note Deleted. Bill No : "
												+ (Long)creditNoteCombo.getValue()+ ", Ledger. : "+ ledgerCombo.getItemCaption(ledgerCombo.getValue())
												+ ", Payment Amount : "
												+ table.getColumnFooter(TBC_AMOUNT).toString(),(Long)creditNoteCombo.getValue());
					...
					if (creditNoteCombo.getValue() != null && !creditNoteCombo.getValue().toString().equals("0")) {
						...
						ConfirmDialog.show(getUI(), "Are you sure?", new ConfirmDialog.Listener() {
							public void onClose(ConfirmDialog dialog) {
								if (dialog.isConfirmed()) {
									try {
										dao.cancel((Long) creditNoteCombo.getValue());
										saveActivity(getOptionId(), "Credit Note Cancelled. Bill No : "
												+ (Long)creditNoteCombo.getValue()+ ", Ledger. : "+ ledgerCombo.getItemCaption(ledgerCombo.getValue())
												+ ", Payment Amount : "
												+ table.getColumnFooter(TBC_AMOUNT).toString(),(Long)creditNoteCombo.getValue());
	...
	public void loadData(long id) {
		...
		try {
			list.add(new CreditNoteModel(0, "----Create New-----"));
			list.addAll(dao.getCreditNoteModelList(getOfficeID()));
			SCollectionContainer bic = SCollectionContainer.setList(list, "id");
			creditNoteCombo.setContainerDataSource(bic);
			creditNoteCombo.setItemCaptionPropertyId("bill_no");
			creditNoteCombo.setValue(id);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public SComboField getAccountDepositNumberList() {
		return creditNoteCombo;
	}

	
	public void setAccountDepositNumberList(SComboField creditNoteCombo) {
		this.creditNoteCombo = creditNoteCombo;
	}
