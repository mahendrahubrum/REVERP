 ...
 *
 */
public class CashAccountPaymentUI extends SparkLogic {
...
//	DocumentAttach docAttach;
	...
	public SPanel getGUI() {
		...
		accountTypeRadio = new SRadioButton(getPropertyName("payment_type"), 200, SConstants.cash_account.cashSupplierList,"key", "value");
			...
			accountTypeRadio.addValueChangeListener(new ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent arg0) {
					try {
						...
						if ((Long) accountTypeRadio.getValue() == SConstants.cash_account.EXPENSE){
							...
							ledgerCombo.setDescription("<i class='ledger_bal_style'>Current Balance : "
									+ ledgerDao
											.getLedgerCurrentBalance((Long) ledgerCombo
													.getValue()) + "</i>");
							if ((Long) accountTypeRadio.getValue() == SConstants.cash_account.SUPPLIER) {
			...
			saveButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					try {
						if (isValid()) {
							if (paymentCombo.getValue() == null || paymentCombo.getValue().toString().equals("0")) {
								CashAccountPaymentModel mdl = new CashAccountPaymentModel();
								List<CashAccountPaymentDetailsModel> childList=new ArrayList<CashAccountPaymentDetailsModel>();
								...
								mdl.setBill_no(getNextSequence("Cash Account Payment", getLoginID(), getOfficeID(), CommonUtil.getSQLDateFromUtilDate(dateField.getValue())));
								mdl.setActive(true);
								...
								while (itr.hasNext()) {
									...
									CashAccountPaymentDetailsModel det=new CashAccountPaymentDetailsModel();
								...
								mdl.setCash_account_payment_list(childList);


								long id = dao.save(mdl,
												tran.getTransaction(SConstants.CASH_ACCOUNT_PAYMENTS,CommonUtil.getSQLDateFromUtilDate(dateField.getValue())),
												invoiceMapList);
								

								saveActivity(getOptionId(), "Cash Account Payment Saved. Bill No : "
				...
				public void valueChange(ValueChangeEvent event) {
					...
					try {
						...
						if (paymentCombo.getValue() != null && !paymentCombo.getValue().toString().equals("0")) {
							...
							CashAccountPaymentModel objModel = dao.getCashAccountPaymentModel((Long) paymentCombo.getValue());
							...
							Iterator it = objModel.getCash_account_payment_list().iterator();
							while (it.hasNext()) {
								CashAccountPaymentDetailsModel det = (CashAccountPaymentDetailsModel) it.next();
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if (isValid()) {
							...
							CashAccountPaymentModel mdl = dao.getCashAccountPaymentModel((Long) paymentCombo.getValue());
							List<CashAccountPaymentDetailsModel> childList=new ArrayList<CashAccountPaymentDetailsModel>();
							List<PaymentInvoiceBean> invoiceMapList=new ArrayList<PaymentInvoiceBean>();
							...
							saveActivity(getOptionId(),"Cash Account Payment Updated. Bill No : "
							...
							public void onClose(ConfirmDialog dialog) {
								if (dialog.isConfirmed()) {
									try {
										...
										saveActivity(getOptionId(),"Cash Account Payment Deleted. Bill No : "
													+ paymentCombo.getItemCaption(paymentCombo.getValue())
													+ ", Cash Acct. : "+ cashAccountSelect.getItemCaption(cashAccountSelect.getValue())
													+ ", Payment Amount : "
				...
				public void buttonClick(ClickEvent event) {
					try {
						...
						if(paymentCombo.getValue()!=null && !paymentCombo.getValue().toString().equals("0")){
							...
							map.put("REPORT_TITLE_LABEL", getPropertyName("cash_account_payment"));
//							map.put("REPORT_SUB_TITLE_LABEL", getPropertyName("cash_account_payment"));
...
//							map.put("NET_PRICE_LABEL", getPropertyName("net_price"));
							...
							double total=0;
							Iterator itr=mdl.getCash_account_payment_list().iterator();
							while (itr.hasNext()) {
								CashAccountPaymentDetailsModel det = (CashAccountPaymentDetailsModel) itr.next();
							...
							report.setReportFileName("Cash Account Payment Print");
	...
	public void loadData(long id) {
		List list= new ArrayList();
		try {
			list.add(new CashAccountPaymentModel(0, "----Create New-----"));
			list.addAll(dao.getCashAccountPaymentModelList(getOfficeID()));
