...
public class CashAccountDepositDao extends SHibernate implements Serializable {
	
	
	@SuppressWarnings("rawtypes")
	public long save(CashAccountDepositModel mdl, TransactionModel transaction, List<PaymentInvoiceBean> invoiceMapList) throws Exception {
		try {
			begin();
			...
			while (itr.hasNext()) {
				...
				mapMdl.setPayment_type(SConstants.CASH_ACCOUNT_DEPOSITS);
	...
	@SuppressWarnings("rawtypes")
	public List getCashAccountDepositModelList(long office) throws Exception {
		List resultList=new ArrayList();
		try {
			begin();
			
			resultList=getSession().createQuery("select new com.inventory.config.acct.model.CashAccountDepositModel(id,bill_no)" +
					" from CashAccountDepositModel where office_id=:office order by id DESC")
	...
	public CashAccountDepositModel getCashAccountDepositModel(long id) throws Exception {
		CashAccountDepositModel mdl=null;
		try {
			begin();
			mdl=(CashAccountDepositModel) getSession().get(CashAccountDepositModel.class, id);
			commit();
		} catch (Exception e) {
			rollback();
			close();
	...
	public void update(CashAccountDepositModel mdl, TransactionModel transaction, List<PaymentInvoiceBean> invoiceMapList) throws Exception {
		try {
			begin();
			...
			oldChildList=getSession().createQuery("select b.id from CashAccountDepositModel a join a.cash_account_deposit_list b where a.id=:id")
			...
			if(oldChildList.size()>0){
				getSession().createQuery("delete from CashAccountDepositDetailsModel where id in (:list)")
	...
	public void delete(long id) throws Exception {
		try {
			begin();
			CashAccountDepositModel mdl=(CashAccountDepositModel) getSession().get(CashAccountDepositModel.class, id);
	...
	public void cancel(long id) throws Exception {
		try {
			...
			getSession().delete(transaction);
			flush();
			getSession().createQuery("update CashAccountDepositModel set active=false where id=:id").setParameter("id", mdl.getId()).executeUpdate();
			commit();
		} catch (Exception e) {
			rollback();
			close();
