 ...
 *         Aug 26, 2013
 */
public class CustomerPaymentsUI extends SparkLogic {
				...
				if (customerComboField.getValue() != null) {
					try {
						...
						if (led.getAddress() != null
							...
							sendWindow = new ComposeMailUI(led.getAddress().getEmail());
						else
			...
			public void buttonClick(ClickEvent event) {
				...
				try {
					if (isValid()) {
						...
						String messageActivity="Updated Customer Receipt. Bill No : "
								+ paymentModel.getPayment_id()
								+ ", Customer : "
								+ customerComboField
										.getItemCaption(customerComboField
						...
						message.append("<tr><td  bgcolor=lightgrey> Option </td><td> Customer Payments </td></tr>");
						...
						message.append("<tr><td  bgcolor=lightgrey> Customer </td><td> "+ customerComboField.getItemCaption(customerComboField.getValue())+" </td></tr>");
							...
							public void onClose(ConfirmDialog dialog) {
								if (dialog.isConfirmed()) {
									try {
										...
										loadPaymentNo(0);
										SNotification
												.show(getPropertyName("delete_success"),
														Type.WARNING_MESSAGE);
				...
				try{
					if(paymentIdComboField.getValue()!=null && !paymentIdComboField.getValue().toString().equals("0")){
						...
						if (reportList.size() > 0) {
							report.setJrxmlFileName(getBillName(SConstants.bills.CUSTOMER_RECEIPT));
