 ...
 *         Aug 23, 2013
 */
public class PaymentDao extends SHibernate implements Serializable {
	...
	public long saveCustomerPayment(PaymentModel paymentModel,
		...
		try {
			...
			while (it.hasNext()) {
				id=it.next();
				double bal_amt=(Double) getSession().createQuery("select amount-payment_amount-paid_by_payment from SalesModel where id=:id")
						.setLong("id", id).uniqueResult();
				
				if(tot_amt>bal_amt) {
					getSession().createQuery("update SalesModel set payment_done='Y',paid_by_payment=paid_by_payment+:amt where id=:id")
					...
					getSession().createQuery("update SalesModel set paid_by_payment=paid_by_payment+:amt where id=:id")
						.setLong("id", id).setDouble("amt", tot_amt).executeUpdate();
