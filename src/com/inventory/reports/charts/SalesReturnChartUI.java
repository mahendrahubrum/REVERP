 ...
 *         Aug 20, 2014
 */
public class SalesReturnChartUI extends SContainerLayout {

	private static final long serialVersionUID = -9139632320642769003L;
	...
	public void getChart(Date fromDate, Date toDate) {
		...
		conf.setSeries(getSalesReturnDetails(fromDate,toDate));
	...
	protected DataSeries getSalesReturnDetails(Date fromDate, Date toDate) {
		...
		try {
			list.addAll(dao.getSalesRetrunChartDetails(CommonUtil.getSQLDateFromUtilDate(fromDate),CommonUtil.getSQLDateFromUtilDate(toDate),getOfficeID()));
...
//				seriesItem.setSelected(true);
				seriesItem.setColor(ChartOptions.get().getTheme().getColors()[0]);
