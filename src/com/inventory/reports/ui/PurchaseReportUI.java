 ...
 *         Aug 8, 2013
 */
public class PurchaseReportUI extends SparkLogic {
	...
	public SPanel getGUI() {
		...
		try {
			...
			purchaseNoComboField = new SComboField(
					...
					"purchase_no", false, getPropertyName("all"));
			...
			generateButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						PurchaseInventoryDetailsModel inventoryDetailsModel = null;
						...
						for (int i = 0; i < purchaseModelList.size(); i++) {
							noData = false;
							purchaseModel = (PurchaseModel) purchaseModelList
									.get(i);
							List<PurchaseInventoryDetailsModel> detailsList = purchaseModel
									.getPurchase_details_list();
							...
							if(purchaseModel.getCurrency_id() == getCurrencyID()){
								reportBean = new PurchaseReportBean(
										...
										String.valueOf(purchaseModel
												.getPurchase_no()),
										purchaseModel.getOffice().getName(), items,
										purchaseModel.getAmount(), purchaseModel
												.getPaymentAmount());
								...
								reportBean = new PurchaseReportBean(
										...
										String.valueOf(purchaseModel
												.getPurchase_no()),
										purchaseModel.getOffice().getName(), items,
										roundNumber(amount), purchaseModel
												.getPaymentAmount());			
								reportBean.setCurrency(getCurrencyDescription(getCurrencyID())+
										" ("+purchaseModel.getAmount()+" "+getCurrencyDescription(purchaseModel.getCurrency_id())+")");
							}
						...
						if (!noData) {
							...
							report.setReportFileName("PurchaseReport");
							report.setReportTitle(getPropertyName("purchase_report"));
...
//							if(bean.getAmount() == bean.getAmountInBaseCurrency()){
//								row = new Object[] {
//										i + 1,
//										bean.getCustomerId(),
//										bean.getCustomer(),
//										roundNumber(bean.getAmount())+" "+getCurrencyDescription(bean.getCurrencyId())};
//								
//							} else {
//														
//							}
...
//								form.addComponent(new SLabel(getPropertyName("shipping_charge"),objModel.getShipping_charge() + ""));
							amount = objModel.getAmount() / objModel.getConversionRate();
							if(objModel.getCurrency_id() == getCurrencyID()){
								...
								form.addComponent(new SLabel(getPropertyName("net_amount"), 
										...
										+" ("+roundNumber(objModel.getAmount())+" "+getCurrencyDescription(objModel.getCurrency_id())+")"));									
							}
					...
					try{
						if (table.getValue() != null) {
							...
							if (reportList != null && reportList.size() > 0) {
								List<PurchaseInventoryDetailsModel> detailsList;
								for (int i = 0; i < reportList.size(); i++) {
									purchaseModel = (PurchaseModel) reportList.get(i);
									...
									if(purchaseModel.getCurrency_id() == getCurrencyID()){
										row = new Object[] {
												i + 1,
												purchaseModel.getId(),
												purchaseModel.getPurchase_no() + "",
												CommonUtil.getUtilFromSQLDate(purchaseModel
														.getDate()) + "",
												purchaseModel.getSupplier().getName(),
												roundNumber(purchaseModel.getAmount())+" "+getCurrencyDescription(getCurrencyID()), items };
									} else {
										row = new Object[] {
												i + 1,
												purchaseModel.getId(),
												...
												purchaseModel.getSupplier().getName(),
												roundNumber(amount)+" "+getCurrencyDescription(getCurrencyID())
												+" ("+roundNumber(purchaseModel.getAmount())+" "+getCurrencyDescription(purchaseModel.getCurrency_id())+")", items };
									}
	...
	protected List<Object> getPurchaseReportList() {
		...
		try {
			purchaseModelList = new PurchaseReportDao()
					.getPurchaseDetails(purchaseNo, supplierId, CommonUtil
	...
	protected List<Object> getPurchaseReportSubList(long id) {
		...
		try {
			purchaseModelList = new PurchaseReportDao()
					.getPurchaseDetails(purchaseNo, id, CommonUtil
							.getSQLDateFromUtilDate(fromDateField.getValue()),
							CommonUtil.getSQLDateFromUtilDate(toDateField
									.getValue()), toLong(officeComboField
									.getValue().toString()),
	...
	protected List<Object> showPurchaseReportList() {
		...
		try {
			purchaseModelList = new PurchaseReportDao()
					.getPurchaseDetailsConsolidated(purchaseNo, supplierId, CommonUtil
							.getSQLDateFromUtilDate(fromDateField.getValue()),
							CommonUtil.getSQLDateFromUtilDate(toDateField
									.getValue()), toLong(officeComboField
									.getValue().toString()),
		...
		try {
			...
			if (supplierId != 0) {
				purchaseBillList = new PurchaseDao()
						.getAllPurchaseNumbersForSupplier(officeId, supplierId,
				...
				purchaseBillList = new PurchaseDao()
						.getAllPurchaseNumbersFromDate(officeId, CommonUtil
			...
			purchaseModel
					.setPurchase_no("------- "+getPropertyName("all")+" --------");
			...
			purchaseNoComboField.setItemCaptionPropertyId("purchase_no");
