...
import com.inventory.reports.dao.SalesManWiseSupplierLedgerReportDao;
 ...
 *         Dec 10, 2013
 */
public class SalesManWiseSupplierLedgerReportUI extends SparkLogic {
	...
	private SalesManWiseSupplierLedgerReportDao daoObj;
	...
	public SPanel getGUI() {
		...
		try {
			...
			table.addContainerProperty(TBC_ID, Long.class, null, TBC_ID, null,
					Align.CENTER);
			table.addContainerProperty(TBC_INVOICE_NO, Long.class, null,
			...
			table.setColumnWidth(TBC_SUPPLIER, 200);
			...
			salesManSelect = new SComboField(getPropertyName("sales_man"), 200,
					new UserManagementDao()
							.getAllLoginsFromOffice(getOfficeID()), "id",
					"login_name");
			...
			daoObj = new SalesManWiseSupplierLedgerReportDao();
...
//							AddCommissionSale commSales = new AddCommissionSale();
...
//							commSales.addCloseListener(closeListener);
							...
							payment.setCaption("Supplier payment");
						...
						public void valueChange(ValueChangeEvent event) {
							...
							try {
								...
								bic1 = SCollectionContainer.setList(
										new UserManagementDao()
												.getAllLoginsFromOffice(getOfficeID()),
										"id");
							...
							salesManSelect
									.setItemCaptionPropertyId("login_name");
				...
				public void valueChange(ValueChangeEvent event) {
					...
					if (salesManSelect.getValue() != null) {
						...
						try {
							...
							lst.addAll(ledDao.getAllSuppliersUnderSalesMan(
		...
		try {
			...
			if (isValid()) {
				...
				if (ledgertSelect.getValue() != null)
					led_id = (Long) ledgertSelect.getValue();
				...
				List lst = daoObj.getNewSupplierLedgerReport(
						...
						getOfficeID(), led_id, usePaymentEndDate.getValue(),
						(Long) salesManSelect.getValue());
				...
				double opening_bal = daoObj.getPurchaseOpeningBalance(
						CommonUtil.getSQLDateFromUtilDate(fromDate.getValue()),
						(Long) officeSelect.getValue(), led_id,
						usePaymentEndDate.getValue(),
						(Long) salesManSelect.getValue());

				if (led_id != 0) {
					LedgerModel ledger = ledDao.getLedgeer(led_id);
						...
						if (obj.getParticulars().equals("Purch Return")) {
							...
							table.addItem(
									new Object[] { ct + 1, obj.getId(),
											obj.getParticulars(),
											obj.getDate(), obj.getNumber(),
											obj.getCustomer(), 0.0, 0.0,
	...
	protected void generateReport() {
		try {
			...
			if (isValid()) {
				...
				double ledger_bal = new LedgerViewDao().getOpeningBalance(
						getWorkingDate(), led_id);

				String led_name = "";
				if (led_id != 0) {
					LedgerModel ledger = ledDao.getLedgeer(led_id);
//					opening_bal += ledger.getOpening_balance();
					led_name = ledger.getName();
				}
				...
				if (reportList != null && reportList.size() > 0) {
					...
					params.put("LedgerName", led_name);
					...
					report.setJrxmlFileName("SalesManWiseSupplierLedgerReport");
					report.setReportFileName("Sales Man Wise Supplier Ledger Report");
					report.setReportTitle("Sales Man Wise Supplier Ledger Report");
...
//							.getAddressString(ledger.getAddress().getId()));
					...
					report.setReportTitle("Supplier Ledger");
					report.setReportSubTitle("From  : "
							+ CommonUtil.formatDateToCommonFormat(fromDate
									.getValue())
							+ "   To  : "
