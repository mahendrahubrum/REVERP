...
public class PurchaseInquiryReportUI extends SparkLogic {
	...
	private SComboField purchaseInquiryNoComboField;
	...
	static String TBC_INQUIRY_NO = "Inquiry No";
...
//	poDao = new PurchaseOrderDao();
		...
		allColumns = new String[] { TBC_SN, TBC_ID, TBC_INQUIRY_NO, TBC_DATE,
				TBC_SUPPLIER, TBC_AMOUNT, TBC_ITEMS };
		visibleColumns = new String[] { TBC_SN, TBC_INQUIRY_NO, TBC_DATE,
				TBC_SUPPLIER, TBC_AMOUNT, TBC_ITEMS };
		...
		try {
			...
			table.addContainerProperty(TBC_ID, Long.class, null, TBC_ID, null,
					Align.CENTER);
			table.addContainerProperty(TBC_INQUIRY_NO, String.class, null,
			...
			table.setColumnExpandRatio(TBC_INQUIRY_NO, 0.9f);
...
final Action actionDelete = new Action("Edit");
				...
				public void handleAction(Action action, Object sender, Object target) {
					try{
						if (table.getValue() != null) {
							...
							PurchaseInquiryUI purchase = new PurchaseInquiryUI();
							purchase.setCaption("Purchase Inquiry");
			...
			generateButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						boolean noData = true;
						PurchaseInquiryModel purchaseModel = null;
						PurchaseInquiryDetailsModel inventoryDetailsModel = null;
						PurchaseReportBean reportBean = null;
						String items = "";

						List<Object> reportList = new ArrayList<Object>();
						List<Object> purchaseModelList = getPurchaseInquiryReportList();
						double amount;
						for (int i = 0; i < purchaseModelList.size(); i++) {
							noData = false;
							purchaseModel = (PurchaseInquiryModel) purchaseModelList
									.get(i);
							List<PurchaseInquiryDetailsModel> detailsList = purchaseModel
									.getInquiry_details_list();
							items = "";
						...
						if (!noData) {
							...
							report.setReportFileName("Purchase Inquiry Report");
							report.setReportTitle(getPropertyName("purchase_inquiry_report"));
							String subHeader = "";
							if (supplierid != 0) {
							...
							if (purchaseInquiryNoComboField.getValue() != null
									&& !purchaseInquiryNoComboField.getValue().equals(
											"")
									&& !purchaseInquiryNoComboField.getValue()
											.toString().equals("0")) {
								subHeader += getPropertyName("inquiry_no")+" : "
										+ purchaseInquiryNoComboField
												.getItemCaption(purchaseInquiryNoComboField
														.getValue());
							}
			...
			showButton.addClickListener(new ClickListener() {
				...
				@Override
				public void buttonClick(ClickEvent event) {
					PurchaseInquiryModel purchaseInquiryModel = null;
					PurchaseInquiryDetailsModel inquiryDetailsModel = null;
					String items = "";
					Object[] row;

					List repList = getPurchaseInquiryReportList();
					...
					if (repList != null && repList.size() > 0) {
						double amount;
						List<PurchaseInquiryDetailsModel> detailsList;
						for (int i = 0; i < repList.size(); i++) {
							purchaseInquiryModel = (PurchaseInquiryModel) repList.get(i);
							detailsList = purchaseInquiryModel
									.getInquiry_details_list();
							items = "";
							for (int k = 0; k < detailsList.size(); k++) {
							...
							amount = purchaseInquiryModel.getAmount() / purchaseInquiryModel.getConversionRate();
							if(purchaseInquiryModel.getCurrencyId() == getCurrencyID()){
								row = new Object[] {
										i + 1,
										purchaseInquiryModel.getId(),
										purchaseInquiryModel.getInquiry_no()
												+ "",
										CommonUtil
												.formatDateToCommonDateTimeFormat(purchaseInquiryModel
														.getDate()),
										purchaseInquiryModel.getSupplier().getName(),
										purchaseInquiryModel.getAmount()+" "+getCurrencyDescription(getCurrencyID()),
										items };
							} else {
								row = new Object[] {
										i + 1,
										...
										" ("+purchaseInquiryModel.getAmount()+" "+getCurrencyDescription(purchaseInquiryModel.getCurrencyId())+")",
										items };
							}
...
*/
	...
	protected List<Object> getPurchaseInquiryReportList() {
		long purchaseNo = 0;
		long supplierId = 0;

		if (purchaseInquiryNoComboField.getValue() != null
				&& !purchaseInquiryNoComboField.getValue().equals("")
				&& !purchaseInquiryNoComboField.getValue().toString().equals("0")) {
			purchaseNo = toLong(purchaseInquiryNoComboField.getValue().toString());
		}
		if (supplierComboField.getValue() != null
		...
		try {
			purchaseModelList = new PurchaseReportDao()
					.getPurchaseInquiryDetails(purchaseNo, supplierId, CommonUtil
	...
	private void loadBillNo(long supplierId, long officeId) {
		List<PurchaseInquiryModel> purchaseBillList = null;
		try {

			if (supplierId != 0) {
				purchaseBillList = purchaseReportDao
						.getAllPurchaseInquiryForSupplier(supplierId, officeId,
								CommonUtil.getSQLDateFromUtilDate(fromDateField.getValue()),
								CommonUtil.getSQLDateFromUtilDate(toDateField.getValue()));
			} else {
				purchaseBillList = purchaseReportDao
						.getAllPurchaseInquiryForOffice(officeId,
								CommonUtil.getSQLDateFromUtilDate(fromDateField.getValue()),
								CommonUtil.getSQLDateFromUtilDate(toDateField.getValue()));
			}

			PurchaseInquiryModel purchaseModel = new PurchaseInquiryModel();
			purchaseModel.setId(0);
			purchaseModel
					.setInquiry_no("--------"+getPropertyName("all")+"--------");
			if (purchaseBillList == null) {
				purchaseBillList = new ArrayList<PurchaseInquiryModel>();
			}
			purchaseBillList.add(0, purchaseModel);
			container = CollectionContainer.fromBeans(purchaseBillList, "id");
			purchaseInquiryNoComboField.setContainerDataSource(container);
			purchaseInquiryNoComboField.setItemCaptionPropertyId("inquiry_no");
			purchaseInquiryNoComboField.setValue(0);
