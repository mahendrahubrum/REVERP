...
public class SalesManWiseSalesReportUI extends SparkLogic {
	...
	public SPanel getGUI() {
		...
		allColumns = new String[] { TBC_SN, TBC_ID,TBC_SALES_MAN, TBC_SALE_NO, TBC_DATE,
				TBC_CUSTOMER, TBC_ITEMS ,TBC_AMOUNT, TBC_PAYMENT, TBC_PENDING };
		visibleColumns = new String[] { TBC_SN, TBC_SALES_MAN,TBC_SALE_NO, TBC_DATE,
				TBC_CUSTOMER, TBC_ITEMS,TBC_AMOUNT, TBC_PAYMENT,TBC_PENDING };
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (!noData) {
							...
							report.setJrxmlFileName("SalesManWiseSalesReport");
							report.setReportFileName("SalesManWiseSalesReport");
					...
					if (repList != null && repList.size() > 0) {
						...
						for (int i = 0; i < repList.size(); i++) {
							...
							try {
								row = new Object[] {
										i + 1,
										salesModel.getId(),userDao.getUser(salesModel.getResponsible_employee()).getFirst_name(),
				...
				public void valueChange(ValueChangeEvent event) {
					try{
						if (table.getValue() != null) {
							...
							form.addComponent(new SLabel(getPropertyName("sales_man"),userDao.getUser(sale.getResponsible_employee()).getFirst_name()));
	...
	protected List<Object> getSalesReportList() {
		...
		try {
			salesModelList = new SalesReportDao()
					.getSalesManWiseSalesDetails(salesNo, custId, CommonUtil
	...
	private void loadBillNo(long customerId, long officeId) {
		...
		try {
			...
			if(salesManComboField.getValue()!=null&&!salesManComboField.getValue().equals("")&&!salesManComboField.getValue().toString().equals("0")){
				condition1 += " and responsible_employee= "+toLong(salesManComboField.getValue().toString());
			}
