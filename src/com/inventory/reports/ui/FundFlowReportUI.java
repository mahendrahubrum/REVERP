...
import com.inventory.reports.dao.FundFlowReportDao;
 ...
 */
public class FundFlowReportUI extends SparkLogic {
			...
			officeComboField.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					try {
						...
						list.addAll(dao.getBankLedgers((Long)officeComboField.getValue()));
				...
				public void handleAction(Action action, Object sender, Object target) {
					try{
						if (table.getValue() != null) {
							...
							long id=dao.getIdFromTransaction((Integer)item.getItemProperty(TBC_TYPE).getValue(), (Long) item.getItemProperty(TBC_ID).getValue());
							if(id!=0){
...
//														sales.loadOptions(id);
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (officeComboField.getValue() != null) {
							...
							resultList = dao.getFundFlowReport((Long) organizationComboField.getValue(),
							...
							if (resultList != null && resultList.size() > 0) {
								...
								try {
									bal=dao.getFundFlowOpeningBalance((Long) officeComboField.getValue(),
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (officeComboField.getValue() != null) {
							...
							reportList = dao.getFundFlowReport((Long) organizationComboField.getValue(),
																(Long) officeComboField.getValue(),
																(Long) ledgerComboField.getValue(),0,0,
																CommonUtil.getSQLDateFromUtilDate(fromDateField.getValue()), 
																CommonUtil.getSQLDateFromUtilDate(toDateField.getValue()),
							...
							if (reportList.size() > 0) {
								try {
									opening_balance=dao.getFundFlowOpeningBalance((Long) officeComboField.getValue(),
																		(Long) ledgerComboField.getValue(),
																		CommonUtil.getSQLDateFromUtilDate(fromDateField.getValue()),
																		settings);
								} catch (Exception e) {
							...
							if (reportMainList.size() > 0) {
								...
								report.setReportFileName("Fund Flow Report");
								...
								map.put("REPORT_TITLE_LABEL", getPropertyName("fund_flow_report"));
	...
	public void showDetails(int type,long tid) {
		...
		try {
			long id=dao.getIdFromTransaction(type, tid);
			if(id!=0){
