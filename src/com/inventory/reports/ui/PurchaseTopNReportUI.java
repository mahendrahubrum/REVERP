...
public class PurchaseTopNReportUI extends SparkLogic {
...
*/
		...
		showButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (isValid()) {
					...
					try {
						list = getPurchaseTopNReportList();
					} catch (Exception e) {
				...
				if (isValid()) {
					try {
						...
						for (TopNReportBean bean : list) {
							...
							isDataExist = true;
							if(reportType == PurchaseTopNReportUI.AMOUNT_WISE){
								bean.setUnit(getCurrencyDescription(getCurrencyID()));
							}
							reportList.add(bean);
							no_of_row--;
						...
						if (isDataExist) {
							...
							report.setReportFileName("PurchaseTopNReport");
							report.setReportTitle(getPropertyName("purchase_top_n_report"));
	...
	private List<TopNReportBean> getPurchaseTopNReportList()
			throws Exception {
		List<TopNReportBean> list = topNReportDao.getPurchaseTopNList(
				getOrganizationID(),
