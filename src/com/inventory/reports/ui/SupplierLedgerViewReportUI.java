 ...
 *         Dec 10, 2013
 */
public class SupplierLedgerViewReportUI extends SparkLogic {
	...
	public SPanel getGUI() {
		...
		try {
			...
			allColumns = new String[] { TBC_SN, TBC_ID, TBC_TYPE, TBC_DATE,
					TBC_SALE, TBC_EXPENSE, TBC_CASH, TBC_RETURN,TBC_PERIOD_BAL, TBC_BALANCE };
			visibleColumns = new String[] { TBC_SN, TBC_DATE, TBC_SALE, TBC_CASH,TBC_BALANCE,TBC_TYPE };
			...
			table.addContainerProperty(TBC_SALE, Double.class, null,getPropertyName("credit"), null, Align.RIGHT);
			table.addContainerProperty(TBC_EXPENSE, Double.class, null,getPropertyName("expenses"), null, Align.RIGHT);
			table.addContainerProperty(TBC_CASH, Double.class, null,getPropertyName("debit"), null, Align.RIGHT);
...
//							commSales.addCloseListener(closeListener);
						...
						getUI().getCurrent().addWindow(pdcPay);
							...
							BankAccountPaymentUI bankDep = new BankAccountPaymentUI();
							bankDep.setCaption(getPropertyName("bank account payment"));
...
//								form.addComponent(new SLabel(getPropertyName("description"),objModel.getDescription()));
								...
								.equals("Cash Account Payment")) {
						CashAccountPaymentModel cashDepMdl = new CashAccountPaymentDao()
						.getCashAccountPaymentModel(id);
						
						SFormLayout form = new SFormLayout();
						form.addComponent(new SHTMLLabel(null,
								"<h2><u>"+getPropertyName("Cash Payment")+"</u></h2>"));
...
//										getPropertyName("cash_cheque"), "Cash"));
						...
						grid.setColumns(5);
						grid.setRows(cashDepMdl.getCash_account_payment_list()
								.size() + 3);
						...
						CashAccountPaymentDetailsModel cashDepDet;
						Iterator itmItr = cashDepMdl.getCash_account_payment_list().iterator();
						int i = 1;
						while (itmItr.hasNext()) {
							cashDepDet = (CashAccountPaymentDetailsModel) itmItr
									.next();
...
//										getPropertyName("cash_cheque"), "Cash"));
						...
						pop.setHideOnMouseOut(false);
							...
							.equals("Purchase Return")) {
								...
								form.addComponent(new SLabel(
										getPropertyName("return_no"),
										objModel.getReturn_no() + ""));
								form.addComponent(new SLabel(
										getPropertyName("supplier"), objModel
												.getSupplier().getName()));
								form.addComponent(new SLabel(
...
//				opening_bal += ledger.getOpening_balance();
				...
				if (reportList != null && reportList.size() > 0) {
					...
					params.put("PURCHASE_LABEL", getPropertyName("credit"));
					params.put("EXPENSE_COMMISSION_LABEL", getPropertyName("expense_commission"));
					params.put("CASH_LABEL", getPropertyName("debit"));	
					...
					report.setJrxmlFileName("SupplierLedgerView");
