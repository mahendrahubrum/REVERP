...
import com.inventory.config.stock.ui.ItemDailyRatesUI;
import com.inventory.reports.dao.DailyRateReportDao;
...
/**
 * @author Jinshad P.T. Inventory Dec 28, 2013
 */
public class DailyRateReportUI extends SparkLogic {
	...
	public SPanel getGUI() {
		allColumns = new Object[] { TBC_SN, TBC_ID,TBC_DATE ,TBC_EMPLOYEE,TBC_SUPPLIER, TBC_ITEM,TBC_UNIT,TBC_RATE};
		visibleColumns = new Object[] { TBC_SN,TBC_DATE ,TBC_EMPLOYEE,TBC_SUPPLIER, TBC_ITEM,TBC_UNIT,TBC_RATE};
		...
		table.addContainerProperty(TBC_SUPPLIER, String.class, null,getPropertyName("customer"), null, Align.LEFT);
				...
				public void handleAction(Action action, Object sender, Object target) {
					try{
						if (table.getValue() != null) {
							...
							ItemDailyRatesUI option=new ItemDailyRatesUI();
							option.setCaption(getPropertyName("daily_quotation"));
							option.getCustomerComboField().setValue((Long) item.getItemProperty(TBC_ID).getValue());
							option.center();
				...
				public void valueChange(ValueChangeEvent event) {
					try{
						if (table.getValue() != null) {
							...
							form.addComponent(new SHTMLLabel(null,"<h2><u>"+getPropertyName("item_rate")+"</u></h2>"));
							form.addComponent(new SLabel(getPropertyName("employee"),itm.getItemProperty(TBC_EMPLOYEE).getValue().toString()));
							form.addComponent(new SLabel(getPropertyName("customer"),itm.getItemProperty(TBC_SUPPLIER).getValue().toString()));
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (officeComboField.getValue() != null) {
							...
							reportList = daoObj.showDailyRateReport(
									itemID,
									custId,
									CommonUtil
							...
							if(reportList.size()>0){
								...
								SNotification.show(getPropertyName("no-data_available"),Type.WARNING_MESSAGE);
							}
				...
				public void buttonClick(ClickEvent event) {
					...
					try {
						...
						if (officeComboField.getValue() != null) {
							...
							reportList = daoObj.getDailyRateReport(
									itemID,
									custId,
									CommonUtil
											.getSQLDateFromUtilDate(fromDateField
							...
							if (reportList.size() > 0) {
								report.setJrxmlFileName("DailyRate_Report");
								report.setReportFileName("Daily Rate Report");
								HashMap<String, Object> map = new HashMap<String, Object>();
								
								map.put("REPORT_TITLE_LABEL", getPropertyName("daily_rate_report"));
