...
import com.inventory.tailoring.dao.TailoringPaymentDao;
 ...
 *         Dec 19, 2014
 */
public class TailoringCustomerPaymentsUI extends SparkLogic {
	...
	private TailoringPaymentDao paymentDao;
	private TailoringSalesDao salesDao;
	...
	private STextField paidTotalAmountField;
	private STextField balanceAmountField;
	...
	public SPanel getGUI() {
		...
		salesDao = new TailoringSalesDao();

		if (session.getAttribute("settings") != null)
			settings = (SettingsValuePojo) session.getAttribute("settings");

		paymentDao = new TailoringPaymentDao();
		...
		paidTotalAmountField = new STextField("Paid Amount", 200);
		paidTotalAmountField.setStyleName("textfield_align_right");
		paidTotalAmountField.setValue("0.00");
		paidTotalAmountField.setReadOnly(true);

		balanceAmountField = new STextField("Balance Amount", 200);
		balanceAmountField.setStyleName("textfield_align_right");
		balanceAmountField.setValue("0.00");
		balanceAmountField.setReadOnly(true);
		...
		customerFormLayout.addComponent(paidTotalAmountField);
		customerFormLayout.addComponent(balanceAmountField);
			...
			public void valueChange(ValueChangeEvent arg0) {
				try {
					...
					if (set != null && set.size() > 0) {
						...
						TailoringSalesModel mdl;
						Iterator itr = set.iterator();
						while (itr.hasNext()) {
							purchaseId = (Long) itr.next();
							mdl = salesDao.getSale(purchaseId);
							amount += mdl.getAmount();
							paidAmount+= mdl.getPayment_amount()+mdl.getPaid_by_payment();
						
						}
						
						customerAmountField.setNewValue(getFormattedAmount(amount-paidAmount));
						saleTotalAmountField.setNewValue(getFormattedAmount(amount));
						paidTotalAmountField.setNewValue(getFormattedAmount(paidAmount));
						balanceAmountField.setNewValue(getFormattedAmount(amount-paidAmount));
				...
				try{
					if(paymentIdComboField.getValue()!=null && !paymentIdComboField.getValue().toString().equals("0")){
						...
						if (reportList.size() > 0) {
							report.setJrxmlFileName("CustomerPayment");
							report.setReportFileName("Customer Payment");
							report.setReportTitle("Customer Payment");
							String subHeader = "";
...
//				groupList.add(SConstants.BANK_ACCOUNT_GROUP_ID);
	...
	protected void loadBillNumbers(Long ledgerId, Date fromDate, Date toDate) {
		...
		try {
			...
			balanceAmountField.setNewValue("0.00");
			paidTotalAmountField.setNewValue("0.00");
			if (ledgerId != null && ledgerId != 0) {
				TailoringSalesModel mdl = null;
				double amount = 0;
				...
				if (paymentIdComboField.getValue() != null
						...
						&& !paymentIdComboField.getValue().toString()
								.equals("0")) {
					lst.addAll(salesDao.getAllSalesIDsForCustomer(ledgerId,
				...
				while (iter.hasNext()) {
					mdl = (TailoringSalesModel) iter.next();
					set.add(mdl.getId());
				...
				paidTotalAmountField.setNewValue(returnAmount + "");
				balanceAmountField.setNewValue(totalAmount + "");
